---
name: dev-flow
description: Orchestrate feature development through delegated implementation, primary-agent final review and testing, and a dedicated commit agent. Use for feature implementation work, including QuantX frontend development; do not use for explanation-only or diagnosis-only requests.
---
...
- Implementation subagents must not stage or commit changes. The final commit is
  owned exclusively by the dedicated commit subagent after approval.
...
2. Split only independent work. Spawn one or more implementation subagents with
   clear ownership and instruct them to inspect existing user changes before
   editing, preserve unrelated work, implement the feature, run focused checks,
   and leave all changes uncommitted.
...
5. Run relevant non-mutating tests, lint, type checks, and builds from the
   ...
   other command that produces tracked implementation changes to an implementation
