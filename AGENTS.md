...
- 优先最小完整实现；跨组件任务在原计划先写明最小整链、验收方式、环境依赖和退出门，先验证一条真实接入路径再扩展同类功能。整链受阻时，只推进不依赖该阻塞且属于已授权范围的工作，并记录未验证边界；不得以组件单测替代整链证据，也不绕过实盘、E2E 或高资源运行授权。不为省当轮 token 留下已知缺陷或省略必要验收。
- 小任务直接完成；长任务在原计划维护简短检查点（已完成、验证及对应代码版本/范围、剩余、阻塞、句柄/日志），不另建轮次报告。续跑从检查点及新增证据恢复，不重新通读历史。阶段完成不等于目标完成，第一版后继续完成必要修复。
- 批量执行独立读取，相关修改形成可验证结果后统一检查；实现稳定后审核和验证。复跑前说明触发原因（新改动、失败或证据失效）及受影响范围；无触发原因不重复测试、审计或格式检查。阶段交付仍须完成项目规定的必要检查。
- 长进程保存句柄和日志，等待已有任务；只读新增结果或日志尾部，不反复列进程、催问、重启检查。进度只报告新证据。
- 同一阻塞没有新证据时不重复探测或解释；一次写清所需决策/外部变化及恢复条件，无独立工作可推进则停止本轮。自动目标续跑遵守宿主的阻塞状态规则，仅做必要状态处理，不把续跑当作新授权。交付时简述整链结果、未完成项和必要的复跑原因，供用户核查。
