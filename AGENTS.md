...
- 除纯讨论、只读排查和代码审查外，每项修改代码、配置或文档的任务都必须从
  `main` 创建独立的短生命周期分支；分支粒度应对应一个可独立验收的变更，而非
  单次对话或单个小步骤。
- 多个 AI 任务并行时，每项任务必须使用各自的分支和 Git worktree，禁止在同一
  工作树中交叉修改或依赖未提交的改动。
- 分支完成后，须先完成适当的验证、最终审核和提交；仅在全部通过后才可合并到
  `main`。`main` 必须始终保持可构建、可测试和可部署。
- 分支创建、worktree 创建、提交及合并均属于 Git 状态变更，必须遵守下一节的
  授权范围和专用 Git 子代理要求。
