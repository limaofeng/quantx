...
from ..resolvers.portfolio_overview import PortfolioOverviewResolver
...
class PortfolioQuery:
  @strawberry.field(description="一次读取账户、持仓、汇总和券商快照元数据")
  async def portfolio_overview(
    self,
    info: strawberry.types.Info,
    account_id: str,
  ) -> PortfolioOverview:
    return await PortfolioOverviewResolver.get_portfolio_overview(
  ...
  @strawberry.field(description="按日期和稳定游标分页获取每日资产快照")
  async def daily_asset_snapshots_page(
    self,
    info: strawberry.types.Info,
    account_id: Optional[str] = None,
    strategy_run_id: Optional[str] = None,
    ...
    return await DailyAssetSnapshotResolver.get_daily_asset_snapshots_page(
      account_id=authorized_account_id(info, account_id),
      strategy_run_id=strategy_run_id,
      scope_type=scope_type,
      start_date=start_date,
