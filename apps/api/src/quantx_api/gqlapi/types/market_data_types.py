...
from quantx_infrastructure.models.market_depth import MarketDepth as DomainMarketDepth
from quantx_infrastructure.models.realtime_price import (
  RealTimePrice as DomainRealTimePrice,
)
...
class IntradayWarmCacheStatus:
  ...
  def from_status(row: dict) -> "IntradayWarmCacheStatus":
    ...
    if isinstance(initialized_date, str):
      initialized_date = date.fromisoformat(initialized_date)
    last_tick_at = row.get("last_tick_at")
    if isinstance(last_tick_at, str):
      last_tick_at = datetime.fromisoformat(last_tick_at.replace("Z", "+00:00"))
    last_kline_at = row.get("last_kline_at")
    if isinstance(last_kline_at, str):
      last_kline_at = datetime.fromisoformat(
        last_kline_at.replace("Z", "+00:00")
      )
