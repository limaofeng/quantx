...
import strawberry
from quantx_infrastructure.models.closed_position_cycle import (
  ClosedPositionCycle as ClosedPositionCycleModel,
)
from quantx_infrastructure.models.daily_asset_snapshot import (
  DailyAssetSnapshot as DailyAssetSnapshotModel,
)
...
class Position:
  ...
  def from_model(
    ...
    resolved_last_price = last_price if last_price is not None else model.last_price
    last_price = (
      float(resolved_last_price) if resolved_last_price is not None else None
    )
...
@strawberry.type(description="每日资产快照游标分页")
class DailyAssetSnapshotPage:
  items: List[DailyAssetSnapshot] = strawberry.field(description="当前页快照")
...
@strawberry.type(description="券商完整持仓快照元数据")
class BrokerPositionSnapshotInfo:
  sequence: str = strawberry.field(description="券商快照单调序列")
  source: str = strawberry.field(description="快照来源")
  reported_at: Optional[datetime] = strawberry.field(description="券商上报时间")
  received_at: Optional[datetime] = strawberry.field(description="服务端接收时间")
  position_count: int = strawberry.field(description="快照持仓数量")
  is_complete: bool = strawberry.field(description="是否为完整快照")
  last_error: Optional[str] = strawberry.field(description="最近一次快照错误")


@strawberry.type(description="账户持仓首屏一致性读模型")
class PortfolioOverview:
  account: Account = strawberry.field(description="账户资产快照")
  positions: List[Position] = strawberry.field(description="同一快照下的持仓")
  summary: PortfolioSummary = strawberry.field(description="基于同一持仓集合的汇总")
  position_snapshot: Optional[BrokerPositionSnapshotInfo] = strawberry.field(
    description="券商持仓快照元数据"
  )
  as_of: datetime = strawberry.field(description="首屏数据时间")
