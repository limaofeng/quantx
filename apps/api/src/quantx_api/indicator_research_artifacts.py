"""Bounded, read-only indicator research projections and exact predicate lookup."""
...
MAX_INDICATOR_METRICS_BYTES = 64 * 1024 * 1024
MAX_INDICATOR_REPORTS = 64
MAX_INDICATOR_ROWS = 12_000
MAX_INDICATOR_MATCH_ENTRIES = 512
MAX_INDICATOR_MATCH_RUNS = 128
MAX_INDICATOR_MATCH_BYTES = 256 * 1024 * 1024
...
_COVERAGE_KEYS = {
  ...
  "historical_st_available",
  "historical_industry_available",
  "restricted_universe",
}
_DISTRIBUTION_KEYS = {
  "indicator_id",
...
def canonical_universe(value: Any) -> dict[str, Any]:
  ...
  if instrument_type not in {"stock", "etf", "stock_and_etf"} or not isinstance(
    exclude_st, bool
  ):
  ...
  return {
    ...
    "include_industries": sorted(
      set(_strings(value.get("include_industries", []), 100))
    ),
    "exclude_industries": sorted(
      set(_strings(value.get("exclude_industries", []), 100))
    ),
  }
...
def _report_projection(report: Any, *, rows: bool) -> dict[str, Any]:
  if not isinstance(report, dict):
    raise ResearchArtifactError("指标报告格式无效")
  report_id = report.get("report_id")
  if not isinstance(report_id, str) or not _SEGMENT_PATTERN.fullmatch(report_id):
    raise ResearchArtifactError("指标报告 ID 格式无效")
  kind = report.get("kind")
  if kind not in {"single", "joint"}:
    raise ResearchArtifactError("指标报告类型无效")
  indicators = sorted(set(_strings(report.get("indicator_ids"), 64)))
  if not indicators or any(
    indicator not in INDICATOR_BY_ID for indicator in indicators
  ):
    raise ResearchArtifactError("指标报告包含未知指标")
  try:
    conditions = normalize_conditions(report.get("conditions", []))
  except (ValueError, TypeError) as exc:
    raise ResearchArtifactError("指标报告条件无效") from exc
  if kind == "single" and (len(indicators) != 1 or conditions):
    raise ResearchArtifactError("单指标总体报告不能包含阈值条件")
  if kind == "joint" and (
    not conditions
    or sorted({item["indicator_id"] for item in conditions}) != indicators
  ):
    raise ResearchArtifactError("联合报告指标与条件不一致")
  ...
  if not isinstance(definitions, list) or len(definitions) > 64:
    raise ResearchArtifactError("指标定义超出边界")
  if not isinstance(distribution, list) or len(distribution) > 512:
    raise ResearchArtifactError("指标分布超出边界")
  if not isinstance(raw_rows, list) or len(raw_rows) > MAX_INDICATOR_ROWS:
    raise ResearchArtifactError("指标统计行超出边界")
  result = {
    "report_id": report_id,
    "kind": kind,
  ...
  relevant_rows = [
    ...
    and (
      row.get("group") == "joint" if kind == "joint" else row.get("group") != "baseline"
    )
  ]
  result["has_adequate_sample"] = any(
    isinstance(row.get("sample_count"), int)
    and row["sample_count"] > 0
    and isinstance(row.get("date_count"), int)
    and row["date_count"] >= 30
    ...
    for row in raw_rows:
      ...
      for key in _ROW_STRINGS:
        if not isinstance(projected.get(key), str):
          raise ResearchArtifactError("指标统计分类字段缺失")
      for key in _ROW_INTS:
        ...
        if (
          ...
          raise ResearchArtifactError("指标统计计数字段无效")
      for key in _ROW_FLOATS:
        value = projected.get(key)
        if value is not None and (
          isinstance(value, bool) or not isinstance(value, (int, float))
        ):
          raise ResearchArtifactError("指标统计数值字段无效")
        projected[key] = value
    ...
    result.update(
      {
        ...
        "distribution": [
          _projection(item, _DISTRIBUTION_KEYS) for item in distribution
        ],
        "rows": safe_rows,
      }
    )
...
def project_indicator_metrics(
  metrics: dict[str, Any], *, rows: bool = False
) -> dict[str, Any]:
  if metrics.get("schema_version") != 1:
    raise ResearchArtifactError("不支持的指标研究产物版本")
  indicator_version = metrics.get("indicator_version")
  if not isinstance(indicator_version, str) or len(indicator_version) > 100:
    raise ResearchArtifactError("缺少指标计算版本")
  ...
  if any(
    isinstance(h, bool) or not isinstance(h, int) or not 1 <= h <= 60 for h in horizons
  ):
    raise ResearchArtifactError("研究周期无效")
  ...
  if not isinstance(reports, list) or len(reports) > MAX_INDICATOR_REPORTS:
    raise ResearchArtifactError("指标报告数量超出边界")
...
def report_reference(summary: ResearchRunRecord, metrics: dict, report: dict) -> dict:
  return {
    ...
    "kind": report["kind"],
    "indicator_ids": report["indicator_ids"],
...
class IndicatorResearchArtifactStore(ResearchArtifactStore):
  def _discover_match_runs(
    self,
  ) -> tuple[list[ResearchRunRecord], int, bool, bool]:
    """Discover only the authoritative indicator-study directory within hard bounds."""
    if not self.root.exists():
      return [], 0, False, False
    if _is_link_like(self.root) or not self.root.is_dir():
      return [], 0, False, True

    study_directory = self.root / "indicator-study-v1"
    ...
    try:
      for run_directory in study_directory.iterdir():
        if len(run_directories) >= MAX_INDICATOR_MATCH_ENTRIES:
          return [], 0, True, False
    ...
    for run_directory in sorted(
      ...
      try:
        ...
        if _is_link_like(manifest_path) or not manifest_path.is_file():
          raise ResearchArtifactError("指标运行清单不是常规文件")
        manifest_bytes = manifest_path.stat().st_size
        if manifest_bytes > _MAX_MANIFEST_BYTES:
          raise ResearchArtifactError("指标运行清单超过大小上限")
        if scanned_bytes + manifest_bytes > MAX_INDICATOR_MATCH_BYTES:
        ...
        if quality_path.exists():
          if _is_link_like(quality_path) or not quality_path.is_file():
            raise ResearchArtifactError("指标数据质量产物不是常规文件")
          quality_bytes = quality_path.stat().st_size
          if quality_bytes > _MAX_DATA_QUALITY_BYTES:
            raise ResearchArtifactError("指标数据质量产物超过大小上限")
          if scanned_bytes + quality_bytes > MAX_INDICATOR_MATCH_BYTES:
            return [], scanned_bytes, True, errors_seen
  ...
  def _metrics(self, summary: ResearchRunRecord, *, rows: bool = False) -> dict:
    ...
    if _is_link_like(path) or not path.is_file():
      ...
      raise ResearchArtifactError("指标产物不是常规文件")
    stat = path.stat()
    if stat.st_size > MAX_INDICATOR_METRICS_BYTES:
      self._invalidate_summary(path)
      raise ResearchArtifactError("指标产物超过大小上限")
    ...
    result = project_indicator_metrics(
      self._read_json(
        summary.run_directory,
        "metrics.json",
        max_bytes=MAX_INDICATOR_METRICS_BYTES,
      ),
      rows=rows,
    )
    after = path.stat()
    if (
      ...
      self._invalidate_summary(path)
      raise ResearchArtifactError("读取期间指标产物发生变化，请刷新")
    if not rows:
      ...
      if size <= _SUMMARY_CACHE_BYTES:
        with _SUMMARY_CACHE_LOCK:
          ...
          _SUMMARY_CACHE.move_to_end(identity)
          while (
            len(_SUMMARY_CACHE) > _SUMMARY_CACHE_LIMIT
  ...
  def get_indicator_report(self, run_key: str, report_id: str) -> dict | None:
    ...
    report = next(
      (item for item in metrics["reports"] if item["report_id"] == report_id), None
    )
    if report is None:
      return None
    errors: list[str] = []
    ...
    return {
      ...
      "indicator_version": metrics["indicator_version"],
      "universe": metrics["universe"],
      "horizons": metrics["horizons"],
      "return_bases": metrics["return_bases"],
      "conditions": report["conditions"],
      "definitions": report["definitions"],
      "coverage": report["coverage"],
      "distribution": report["distribution"],
      "rows": report["rows"],
      "warnings": metrics["warnings"] + report["warnings"],
  ...
  def _safe_config(self, summary: ResearchRunRecord, errors: list[str]) -> dict | None:
    ...
    try:
      raw = yaml.safe_load(
        self._read_text(
          summary.run_directory, "resolved-config.yaml", max_bytes=_MAX_CONFIG_BYTES
        )
      )
      if not isinstance(raw, dict):
        raise ResearchArtifactError("研究配置必须是 object")
      safe = _projection(raw, {"study", "version", "indicator_ids", "date_range"})
      ...
      for key, allowed in {
        ...
        "outcomes": {
          "horizons",
          "include_close_response",
          "include_next_open_return",
          "include_benchmark_excess",
          "include_cross_section_excess",
        },
        "statistics": {
          "bootstrap_method",
          "bootstrap_samples",
  ...
  def match_reports(self, requests: list[dict]) -> list[dict]:
    ...
    results = [
      {
        ...
        "status": "UNSUPPORTED" if item["unsupported"] else "MISSING",
        "reason": "；".join(item["blockers"])
        or "尚无匹配研究报告，可下载配置后运行离线分析命令",
        "reports": [],
        "config_json": item["config"],
        "command": "uv run --no-sync quantx-research run --config indicator-study.json",
    ...
    if all(item["unsupported"] for item in normalized):
      return results
    records, scanned_bytes, budget_exhausted, errors_seen = self._discover_match_runs()
    scanned_runs = 0
    for record in records:
      if record.study_id != "indicator-study" or record.status != "success":
        continue
      if scanned_runs >= MAX_INDICATOR_MATCH_RUNS:
      ...
      try:
        ...
        if metrics_bytes > MAX_INDICATOR_METRICS_BYTES:
          raise ResearchArtifactError("指标产物超过大小上限")
        if scanned_bytes + metrics_bytes > MAX_INDICATOR_MATCH_BYTES:
          budget_exhausted = True
          break
      ...
      try:
        ...
        if scanned_bytes + config_bytes > MAX_INDICATOR_MATCH_BYTES:
          budget_exhausted = True
          break
      ...
      for request, result in zip(normalized, results, strict=True):
        ...
        for report in metrics["reports"]:
          if (
            ...
            or report["indicator_ids"] != request["indicator_ids"]
          ):
            continue
          if (
            report["conditions"] != request["conditions"]
            or metrics["indicator_version"] != INDICATOR_VERSION
          ):
            continue
          ...
          if metrics["horizons"] != DEFAULT_HORIZONS or metrics["return_bases"] != [
            "close",
            "next_open",
          ]:
            reasons.append("研究周期或收益口径不同于默认1–20日双口径")
          if report["coverage"].get("requested_stock_codes") or report["coverage"].get(
            "restricted_universe"
          ):
          ...
          reference["match_reason"] = (
            "；".join(reasons)
            if reasons
            else "条件匹配，但有效日期不足或样本为空"
            if insufficient
            else "条件匹配；请同时查看数据覆盖与统计限制"
          )
          ...
          if result["status"] not in {"MATCHED", "DATA_INSUFFICIENT"} and status in {
            "MATCHED",
            "DATA_INSUFFICIENT",
          }:
      ...
      if all(
        request["unsupported"] or result["status"] in {"MATCHED", "DATA_INSUFFICIENT"}
        for request, result in zip(normalized, results, strict=True)
      ):
        break
    if errors_seen or budget_exhausted:
      for result in results:
        if result["status"] == "MISSING":
          ...
          result["reason"] = (
            "指标报告扫描达到安全预算，未在有界范围内找到匹配报告"
            if budget_exhausted
            else "存在无法安全读取的指标产物，未找到可用匹配报告"
          )
    return results
  ...
  def _request(request: dict) -> dict:
    ...
    if (
      ...
      or kind not in {"single", "joint"}
    ):
      raise ResearchArtifactError("报告请求身份无效")
    indicator_ids = sorted(set(_strings(request.get("indicator_ids"), 64)))
    if not indicator_ids or any(item not in INDICATOR_BY_ID for item in indicator_ids):
      raise ResearchArtifactError("报告请求包含未知指标")
    conditions = normalize_conditions(request.get("conditions", []))
    if kind == "single" and (len(indicator_ids) != 1 or conditions):
      raise ResearchArtifactError("单指标总体报告只能请求一个指标且不能包含阈值")
    if kind == "joint" and (
      not conditions
      or sorted({item["indicator_id"] for item in conditions}) != indicator_ids
    ):
      raise ResearchArtifactError("联合报告指标必须与所有条件一致")
    universe = canonical_universe(request.get("universe"))
    unsupported = [
      INDICATOR_BY_ID[item]
      for item in indicator_ids
      if not INDICATOR_BY_ID[item].research_supported
    ]
