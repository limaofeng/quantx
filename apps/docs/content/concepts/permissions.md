...
| `trade:manual` | Web/iOS 两阶段手动委托预览、确认与撤单 |
...
服务端实时预览
  -> 登录会话绑定、一次性、短时确认挑战
  -> 用户在客户端逐项核对并显式确认
  -> 原生端执行 Face ID / Touch ID
  -> 确认时重新校验全部门禁
```

原生端生物识别和 Web 确认对话框都是本地逐次确认，不替代服务端权限。确认
Token 只在内存短暂保存，绑定用户、登录会话、账户、完整输入指纹和过期时间；
Mutation 成功只表示业务命令进入统一执行链路，不表示券商已报或成交。
