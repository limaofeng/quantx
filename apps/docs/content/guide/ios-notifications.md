...
业务通知由独立 Worker 从数据库真源投影，不依赖 Redis。当前映射只接受可明确
绑定账户的持久化事件：仍待确认的交易意图、已经 `APPLIED` 的委托/成交运行时
事件、账户安全状态事件、失败的自动化命令和 Agent 报告死信。PAPER 与 LIVE
可产生验证通知，BACKTEST 不产生待确认或委托成交通知；请求成功、命令投递和
`command_ack` 均不能生成 `ORDER_UPDATE`。

每个源事件先在同一数据库事务写入全局投影回执，再按投影当时仍有效的用户账户
授权、设备会话和类别偏好创建随机事件及 outbox。回执只保存受控源类型、随机技术
事件 ID 和带服务端密钥的 HMAC，不保存订单、证券、金额或业务 payload。即使当时
没有可用设备或类别处于关闭状态，回执仍会消费该源；之后新增设备或重新开启类别
不会补发旧通知。
