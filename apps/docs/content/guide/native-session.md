...
服务端要求当前用户恰好授权一个资金账户，并在每次认证时从账户授权关系解析；
设备会话和 Access Token 不重复保存账户。零账户或多账户都会使登录失败。响应只通过
...
`admin:*` 等宽泛权限不会进入原生会话。Refresh Token 轮换保持权限上限：用户
权限被收回时会收缩，新增用户权限不会扩大既有设备会话；账户仍按当前唯一授权
关系重新校验。
