"""Strict, reproducible configuration for daily indicator association research."""
...
from quantx_domain.indicators import INDICATOR_DEFINITIONS, normalize_conditions
...
class IndicatorUniverseConfig(UniverseConfig):
  ...
  def historical_filters(self) -> "IndicatorUniverseConfig":
  ...
  def identity(self) -> dict[str, object]:
    # Every field here can change the rows admitted to a indicator report.  Keep
...
class IndicatorOutcomeConfig(OutcomeConfig):
  ...
  @model_validator(mode="after")
  def bounded_horizons(self) -> "IndicatorOutcomeConfig":
    if max(self.horizons) > 60:
      raise ValueError("indicator-study 观察周期不得超过60个交易日")
    return self


class IndicatorStatisticsConfig(StatisticsConfig):
  run_regression: Literal[False] = False


class IndicatorStudyConfig(_StrictModel):
  study: Literal["indicator-study"] = "indicator-study"
  version: Literal["v1"] = "v1"
  indicator_ids: tuple[str, ...] = ()
  conditions: tuple[dict[str, object], ...] = ()
  universe: IndicatorUniverseConfig = Field(default_factory=IndicatorUniverseConfig)
  date_range: tuple[date, date] | None = None
  outcomes: IndicatorOutcomeConfig = Field(default_factory=IndicatorOutcomeConfig)
  statistics: IndicatorStatisticsConfig = Field(
    default_factory=IndicatorStatisticsConfig
  )
  runtime: RuntimeConfig = Field(default_factory=RuntimeConfig)

  @model_validator(mode="after")
  def validate_research(self) -> "IndicatorStudyConfig":
    definitions = {item.id: item for item in INDICATOR_DEFINITIONS}
    normalized = normalize_conditions(list(self.conditions))
    ids = tuple(sorted(set(self.indicator_ids)))
    required = set(ids) | {item["indicator_id"] for item in normalized}
    if not required:
      raise ValueError("indicator_ids 或 conditions 至少指定一个研究对象")
    for indicator_id in required:
      ...
      if not definition.research_supported:
        raise ValueError(f"指标暂不支持历史研究: {indicator_id}")
    ...
    if self.statistics.run_regression:
      raise ValueError("indicator-study 不执行自动回归或权重优化")
    calendar_years = (
    ...
    group_counts = [
      3 if definitions[indicator_id].kind == "binary" else 6 for indicator_id in ids
    ]
    if normalized:
    ...
    if (
      ...
      raise ValueError(
        "研究统计超出页面安全读取上限；请拆分指标批次或减少条件/观察周期/历史年数"
      )
    object.__setattr__(self, "indicator_ids", ids)
  ...
  @property
  def required_indicator_ids(self) -> tuple[str, ...]:
    return tuple(
      sorted(
        set(self.indicator_ids)
        | {str(item["indicator_id"]) for item in self.conditions}
      )
    )

  @property
  def required_lookback(self) -> int:
    wanted = set(self.required_indicator_ids)
    return max(item.lookback for item in INDICATOR_DEFINITIONS if item.id in wanted)
