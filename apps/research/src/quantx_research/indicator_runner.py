"""Read-only indicator-study runner, using the existing audited data sources."""
...
from quantx_research.indicator_study import (
  INDICATOR_REPORT_CHECKPOINT_SCHEMA_VERSION,
  WARNINGS,
  add_indicator_outcomes,
  all_indicator_report_checkpoints_exist,
  analyze_indicator_partitions,
)
...
INDICATOR_STATISTICS_INPUT_SCHEMA_VERSION = 2
INDICATOR_STATISTICS_LEGACY_INPUT_SCHEMA_VERSION = 1
INDICATOR_STATISTICS_ENGINE_IDENTITY_SCHEMA_VERSION = 1
INDICATOR_STATISTICS_ENGINE_VERSION = "indicator-statistics-v2"
INDICATOR_DIVIDEND_COVERAGE_EVIDENCE_SCHEMA_VERSION = 2
...
_INDICATOR_STATISTICS_SOURCE_FILES = (
  (
    "quantx_research.indicator_runner",
    "apps/research/src/quantx_research/indicator_runner.py",
  ),
  (
    "quantx_research.indicator_study",
    "apps/research/src/quantx_research/indicator_study.py",
  ),
  (
    "quantx_research.indicator_config",
    "apps/research/src/quantx_research/indicator_config.py",
  ),
  (
  ...
  (
    "quantx_domain.indicators.screening",
    "packages/domain/src/quantx_domain/indicators/screening.py",
  ),
)


def load_indicator_config(path: str | Path) -> IndicatorStudyConfig:
  payload = yaml.safe_load(Path(path).read_text(encoding="utf-8"))
  if not isinstance(payload, dict):
    raise ValueError("研究配置根节点必须是 YAML mapping")
  return IndicatorStudyConfig.model_validate(payload)
...
async def validate_indicator_study(
  ...
  *,
  ...
  config = await _freeze_window(
    load_indicator_config(config_path),
    source=source,
  ...
  try:
    with (
      monitor,
      tempfile.TemporaryDirectory(prefix="validate-indicator-", dir=root) as temporary,
    ):
      async with _research_source(
        source, market_data_archive=market_data_archive
      ) as active:
        features = await stage_indicator_features(
          active, config, Path(temporary), monitor
        )
        quality = dict(features.quality)
      _, quality = finish_indicator_partitions(
        features, config, Path(temporary), monitor
      )
...
async def run_indicator_study(
  ...
  *,
  ...
  if resuming and any(
    ...
    raise ValueError(
      "恢复 indicator-study 只读取冻结样本；不能同时设置 source、"
      "market_data_archive、output_root 或 now"
    )

  statistics_engine = _indicator_statistics_engine_identity()
  statistics_identity: dict[str, Any] | None = None
  if resuming:
    run_dir = _resolve_indicator_resume_directory(Path(resume_run_dir))
    ...
    lease = _acquire_indicator_run_lease(
      run_dir,
  ...
  try:
    with (
      ...
      if resuming:
        ...
        partitions = (
          {}
          if all_indicator_report_checkpoints_exist(
            run_dir / "statistics-checkpoints", config
          )
          else _partition_existing_indicator_sample(sample_path, staging, monitor)
        )
        quality["warnings"] = list(
          dict.fromkeys(
            [
              *quality.get("warnings", []),
              "本次从同一运行的已核验冻结样本恢复统计；未重新读取行情或计算指标/收益。",
            ]
          )
        )
      else:
        async with _research_source(
          ...
          features = await stage_indicator_features(active, config, staging, monitor)
          quality = dict(features.quality)
        partitions, quality = finish_indicator_partitions(
        ...
        statistics_identity = _indicator_statistics_identity(
          config_hash=config_hash,
        ...
        manifest["artifacts"] = _indicator_recovery_artifact_index(
          run_dir,
          sample_sha256=sample_sha256,
        )
        _atomic_write_json(run_dir / "manifest.json", manifest)
      ...
      if statistics_identity is None:
        raise RuntimeError("indicator-study 统计输入身份未初始化")
      ...
      metrics = analyze_indicator_partitions(
        partitions,
        config,
      ...
      if (run_dir / "metrics.json").stat().st_size > 64 * 1024**2:
        raise ResearchPreflightError(
          "结构化指标产物超过页面64MiB安全读取上限；请拆分研究配置",
          run_dir=run_dir,
        )
      ...
      render_indicator_report(run_dir)
    ...
    try:
      ...
      if _indicator_recovery_inputs_committed(manifest):
        ...
        _atomic_write_json(run_dir / "data-quality.json", quality)
      manifest["artifacts"] = _indicator_artifact_index(run_dir, lease_path=lease.path)
      _atomic_write_json(run_dir / "manifest.json", manifest)
    finally:
      lease.release()
  return run_dir
...
class _IndicatorRunLease:
  ...
  def release(self) -> None:
    if self.stream is None:
      return
    try:
      _unlock_indicator_run_lease(self.stream)
    finally:
      self.stream.close()
      self.stream = None


def _acquire_indicator_run_lease(
  run_dir: Path,
  *,
  attempt_id: str,
  previous_status: str,
  ...
  path = run_dir / ".indicator-study-run-lease.json"
  if previous_status == "running":
    ...
    if _attempt_process_is_active(previous):
      raise ValueError("indicator-study 运行仍由活动进程持有，不能接管")

  try:
    stream = path.open("a+b")
  except OSError as exc:
    raise ValueError("indicator-study 运行租约仍由活动进程持有，不能接管") from exc
  try:
    if path.stat().st_size == 0:
      stream.write(b"\0")
      stream.flush()
    _lock_indicator_run_lease(stream)
  except OSError as exc:
    stream.close()
    raise ValueError("indicator-study 运行租约仍由活动进程持有，不能接管") from exc
  ...
  try:
    ...
    _unlock_indicator_run_lease(stream)
    stream.close()
    raise
  return _IndicatorRunLease(path=path, identity=identity, stream=stream)


def _lock_indicator_run_lease(stream: Any) -> None:
  stream.seek(0)
  if os.name == "nt":
    import msvcrt
...
def _unlock_indicator_run_lease(stream: Any) -> None:
  stream.seek(0)
  if os.name == "nt":
    import msvcrt
...
def _load_indicator_resume(
  ...
  *,
  ...
  run_dir = _resolve_indicator_resume_directory(requested_run_dir)

  manifest = _read_json_object(run_dir / "manifest.json", label="运行 manifest")
  if manifest.get("run_id") != run_dir.name:
    raise ValueError("恢复运行目录名与 manifest.run_id 不一致")
  if manifest.get("study_id") != "indicator-study":
    raise ValueError("只能恢复 indicator-study 运行")
  if manifest.get("status") not in {"failed_resource", "failed", "running"}:
    raise ValueError(
      "只允许恢复 failed_resource、failed 或可证明已停止的 running indicator-study 运行"
    )
  if manifest.get("indicator_version") != INDICATOR_VERSION:
    raise ValueError("恢复运行的 indicator_version 与当前实现不一致")
  ...
  saved_config = IndicatorStudyConfig.model_validate(saved_payload)
  requested_config = load_indicator_config(config_path)
  ...
  identity = _indicator_statistics_identity(
    config_hash=config_hash,
    ...
    manifest.setdefault("statistics_identity_upgrades", []).append(
      {
        ...
        "to_statistics_input_schema_version": (
          INDICATOR_STATISTICS_INPUT_SCHEMA_VERSION
        ),
...
def _resolve_indicator_resume_directory(requested_run_dir: Path) -> Path:
  try:
...
def _indicator_statistics_identity(
  *,
  config_hash: str,
  ...
  if (
    ...
    raise ValueError("indicator-study 统计日历身份缺失")
  return {
    "schema_version": INDICATOR_STATISTICS_INPUT_SCHEMA_VERSION,
    "checkpoint_schema_version": INDICATOR_REPORT_CHECKPOINT_SCHEMA_VERSION,
    "config_hash": config_hash,
    "data_fingerprint": data_fingerprint,
...
def _indicator_statistics_engine_identity() -> dict[str, Any]:
  """Bind checkpoints to the exact statistical source and runtime versions."""
  from quantx_research.runner import REPO_ROOT

  source_files: list[dict[str, str]] = []
  for module, relative_path in sorted(_INDICATOR_STATISTICS_SOURCE_FILES):
    ...
    if not loaded_file:
      raise ValueError(f"indicator-study 统计引擎模块未加载: {module}")
    try:
      path = Path(loaded_file).resolve(strict=True)
    except OSError as exc:
      raise ValueError(f"indicator-study 统计引擎源码不可读取: {module}") from exc
    if path != expected_path:
      raise ValueError(
        "indicator-study 统计引擎模块未从预期工作区加载: "
        f"{module} ({path} != {expected_path})"
      )
    if not path.is_file():
      raise ValueError(f"indicator-study 统计引擎源码不存在: {relative_path}")
    source_files.append(
      {
        "module": module,
        "path": relative_path,
  ...
  payload: dict[str, Any] = {
    "schema_version": INDICATOR_STATISTICS_ENGINE_IDENTITY_SCHEMA_VERSION,
    "engine_version": INDICATOR_STATISTICS_ENGINE_VERSION,
...
def _legacy_statistics_identity(identity: dict[str, Any]) -> dict[str, Any]:
  ...
  projected["schema_version"] = INDICATOR_STATISTICS_LEGACY_INPUT_SCHEMA_VERSION
  return projected
...
def _indicator_recovery_inputs_committed(manifest: dict[str, Any]) -> bool:
...
def _indicator_recovery_artifact_index(
  run_dir: Path,
  *,
  sample_sha256: str,
) -> list[dict[str, Any]]:
...
def _partition_existing_indicator_sample(
  sample_path: Path,
  directory: Path,
  monitor: RuntimeMemoryMonitor,
) -> dict[str, list[Path]]:
  ...
  for index, batch in enumerate(
    ...
    monitor.guard(
      "indicator_resume_partition", estimated_increment_bytes=batch.nbytes * 3
    )
    ...
    monitor.checkpoint("indicator_resume_partition")
...
def _validate_resume_dividend_factor_coverage(quality: dict[str, Any]) -> None:
  ...
  if (
    ...
    or schema_version != INDICATOR_DIVIDEND_COVERAGE_EVIDENCE_SCHEMA_VERSION
...
def _read_indicator_metrics(path: Path) -> dict[str, Any]:
  metrics = _read_json_object(path, label="指标研究 metrics")
  reports = metrics.get("reports")
  warnings = metrics.get("warnings")
  if (
    metrics.get("schema_version") != 1
    or metrics.get("study_id") != "indicator-study"
    or not isinstance(reports, list)
    or not reports
    or not isinstance(warnings, list)
  ):
    raise ValueError("指标研究 metrics 结构不完整，拒绝渲染")
  report_ids: list[str] = []
  for report in reports:
    if not isinstance(report, dict):
      raise ValueError("指标研究 metrics 报告结构不完整，拒绝渲染")
  ...
  if len(report_ids) != len(set(report_ids)):
    raise ValueError("指标研究 metrics 含重复 report_id，拒绝渲染")
  return metrics


def _validate_indicator_render_inputs(
  metrics: dict[str, Any], manifest: dict[str, Any]
) -> None:
  for key in ("indicator_version", "config_hash", "data_fingerprint"):
    if metrics.get(key) != manifest.get(key):
      raise ValueError(f"指标研究 metrics.{key} 与 manifest 不一致，拒绝渲染")
  ...
  if (
    ...
    raise ValueError("指标研究报告族不完整或与 manifest 进度不一致，拒绝渲染")


def _replace_indicator_artifact_entry(
...
def render_indicator_existing(run_dir: str | Path) -> Path:
  """Regenerate a completed indicator report under the same exclusive run lease."""
  directory = _resolve_indicator_resume_directory(Path(run_dir))
  ...
  try:
    ...
    if (
      manifest.get("study_id") != "indicator-study"
      or manifest.get("status") != "success"
    ):
      raise ValueError("只能重新渲染已成功完成的 indicator-study 运行")
    metrics_path, _ = _verify_indexed_artifact(directory, manifest, "metrics.json")
    metrics = _read_indicator_metrics(metrics_path)
    _validate_indicator_render_inputs(metrics, manifest)
    report = render_indicator_report(directory, metrics=metrics)
    _replace_indicator_artifact_entry(
      manifest,
      directory,
      "report.html",
    )
...
@dataclass
class _IndicatorFeatureStage:
  paths: list[Path]
  calendar: pd.DatetimeIndex
  quality: dict[str, Any]


def indicator_data_fingerprint(
  panel_sha256: str,
  indicator_calendar: pd.DatetimeIndex,
  ...
  return fingerprint(
    {
      ...
      "indicator_calendar": days(indicator_calendar),
      "outcome_calendar": days(outcome_calendar),
    }
  )


async def build_indicator_partitions(
  ...
  features = await stage_indicator_features(source, config, directory, monitor)
...
async def stage_indicator_features(
  ...
  if provenance.get("kind") == "qmt-daily-bar-archive" and archive_start:
    ...
    # enough observations exist. Preserve missing indicator warmup as NaN; never
  ...
  if indicator_calendar.empty:
    raise ValueError("缺少基准交易日历，无法审计历史指标中的物理行情缺口")
  ...
  counts = {indicator_id: 0 for indicator_id in config.required_indicator_ids}
  for index in range(0, len(sorted_codes), batch_size):
    ...
    monitor.guard("indicator_load_batch", estimated_increment_bytes=estimate)
    ...
    valid = valid_indicator_observations(panel)
    ...
    for _, stock in panel.groupby("stock_code", sort=False):
      stock = stock.copy()
      for indicator_id in counts:
        stock[indicator_id] = np.nan
      ...
      computed = calculate_indicator_frame(
        calculation_input, trading_dates=indicator_calendar
      )
      for indicator_id in counts:
        stock[indicator_id] = pd.to_numeric(
          computed[indicator_id], errors="coerce"
        ).astype(float)
  ...
  quality.update(
    {
      ...
      "data_fingerprint": indicator_data_fingerprint(
        hasher.hexdigest(), indicator_calendar, calendar
      ),
  ...
  if start > requested_source_start:
    quality["warnings"].append(
      "复权和指标预热查询受 archive 起点限制；完整历史窗口不足的指标值保持不可用。"
    )
  return _IndicatorFeatureStage(feature_paths, calendar, quality)


def finish_indicator_partitions(
  features: _IndicatorFeatureStage,
  config: IndicatorStudyConfig,
  directory: Path,
  monitor: RuntimeMemoryMonitor,
) -> tuple[dict[str, list[Path]], dict[str, Any]]:
  ...
  for index, path in enumerate(features.paths):
    ...
    for indicator_id in counts:
      counts[indicator_id] += int(np.isfinite(panel[indicator_id]).sum())
    for month, monthly in panel.groupby(panel["event_date"].dt.strftime("%Y-%m")):
      output = months_dir / f"{month}-{index:05d}.parquet"
      monthly.to_parquet(output, index=False, row_group_size=65_536)
      partitions.setdefault(str(month), []).append(output)
    monitor.checkpoint("indicator_outcomes_written")
  quality = dict(features.quality)
  quality.update(
    {
      "analysis_sample_count": sample_count,
      "indicator_valid_counts": counts,
...
def _preflight_errors(quality: dict[str, Any]) -> list[str]:
  ...
  if not any(quality.get("indicator_valid_counts", {}).values()):
    errors.append("所选指标均缺少足够的历史窗口")
  return errors
    ...
    for month in sorted(partitions):
      for path in partitions[month]:
        for batch in pq.ParquetFile(path).iter_batches(batch_size=65_536):
          monitor.guard(
            "indicator_sample_artifact", estimated_increment_bytes=batch.nbytes * 3
          )
...
def render_indicator_report(
  ...
  metrics = metrics or _read_indicator_metrics(directory / "metrics.json")

  def escape(value: object) -> str:
    return html.escape(str(value), quote=True)

  sections = [
    "<!doctype html><html lang='zh-CN'><meta charset='utf-8'><title>QuantX 指标研究</title>",
    ...
    "<h1>QuantX 指标历史关联研究</h1>",
    f"<p>{escape(metrics['data_start'])} — {escape(metrics['data_end'])} · 定义 {escape(metrics['indicator_version'])}</p>",
