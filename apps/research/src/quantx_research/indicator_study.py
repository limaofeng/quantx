"""Calendar-aligned indicator cohorts and bounded-memory descriptive inference.
...
INDICATOR_REPORT_CHECKPOINT_SCHEMA_VERSION = 1
...
WARNINGS = [
  ...
  "连续指标按每日截面五分位分组；相同值不拆分。分组取值范围是跨日期观测范围，不是固定筛选阈值。",
  "上涨比例提升及均值差异按同日配对、日期等权计算；不同单指标分组的可观察日期可能不同。",
]
...
def indicator_groups(values: pd.Series, *, binary: bool) -> pd.Series:
...
def indicator_report_specs(
  config: IndicatorStudyConfig,
) -> list[tuple[str, list[str], list[dict[str, object]]]]:
  requested = [("single", [indicator_id], []) for indicator_id in config.indicator_ids]
  if config.conditions:
    requested.append(
      (
        "joint",
        sorted({str(item["indicator_id"]) for item in config.conditions}),
...
def all_indicator_report_checkpoints_exist(
  checkpoint_directory: Path,
  config: IndicatorStudyConfig,
) -> bool:
  """Cheap precheck used to skip rebuilding transient month partitions."""
  requested = indicator_report_specs(config)
...
def analyze_indicator_partitions(
  ...
  *,
  ...
  definitions = {item.id: item for item in INDICATOR_DEFINITIONS}
  reports: list[dict[str, Any]] = []
  requested = indicator_report_specs(config)
  if checkpoint_directory is not None:
    checkpoint_directory.mkdir(parents=True, exist_ok=True)
  for index, (kind, indicator_ids, conditions) in enumerate(requested, start=1):
    monitor.checkpoint("indicator_report_start")
    checkpoint_path = (
    ...
    if checkpoint_path is not None and checkpoint_path.is_file():
      ...
      reused = True
      print(
        f"indicator-study report {index}/{len(requested)}: reuse {checkpoint_path.name}",
        flush=True,
      )
    else:
      print(
        f"indicator-study report {index}/{len(requested)}: {kind} {','.join(indicator_ids)}",
        flush=True,
      )
      with tempfile.TemporaryDirectory(
        prefix="indicator-statistics-", dir=staging_directory
      ) as scratch:
      ...
      report["definitions"] = [asdict(definitions[value]) for value in indicator_ids]
...
def apply_indicator_report_fdr(
  reports: list[dict[str, Any]],
  *,
  config: IndicatorStudyConfig,
) -> dict[str, Any]:
...
def _write_report_checkpoint(
  ...
  *,
  ...
  payload = {
    "schema_version": INDICATOR_REPORT_CHECKPOINT_SCHEMA_VERSION,
    "identity": identity,
...
def _load_report_checkpoint(
  ...
  *,
  ...
  try:
    ...
    raise ValueError(f"指标统计检查点不可读: {path}") from exc
  if payload.get("schema_version") != INDICATOR_REPORT_CHECKPOINT_SCHEMA_VERSION:
    raise ValueError(f"指标统计检查点版本不匹配: {path}")
  if (
    payload.get("identity_sha256") != fingerprint(expected_identity)
    or payload.get("identity") != expected_identity
  ):
    raise ValueError(f"指标统计检查点身份不匹配: {path}")
  if payload.get("report_spec") != expected_report_spec:
    raise ValueError(f"指标统计检查点报告定义不匹配: {path}")
  report = payload.get("report")
  if not isinstance(report, dict) or payload.get("report_sha256") != fingerprint(
    report
  ):
    raise ValueError(f"指标统计检查点内容校验失败: {path}")
  return report
...
def _analyze_report(
  ...
  *,
  ...
  binary = {item.id: item.kind == "binary" for item in INDICATOR_DEFINITIONS}
  groups = (
    (
      ["baseline", "false", "true"]
      if binary[indicator_ids[0]]
  ...
  for month in sorted(partitions):
    ...
    monitor.guard(
      "indicator_month_projection",
      estimated_increment_bytes=row_count * len(columns) * 32,
    )
    frame = pd.concat(
    ...
    for day, daily in frame.groupby("event_date", sort=True):
      ...
      valid = np.isfinite(daily[indicator_ids].to_numpy(dtype=float)).all(axis=1)
      ...
      if kind == "single":
        labels = indicator_groups(
          daily[indicator_ids[0]], binary=binary[indicator_ids[0]]
        )
        for group in groups[1:]:
          memberships[group] = labels.eq(group).fillna(False)
          values = daily.loc[memberships[group], indicator_ids[0]]
          if values.empty:
            continue
      ...
      monitor.checkpoint("indicator_daily_statistics")
  ...
  for (group, horizon, basis), cell in cells.items():
    monitor.guard("indicator_exact_median", estimated_increment_bytes=cell.count * 24)
    values = (
  ...
  if not valid_count:
    warnings.append("没有具备全部所需指标值的有效样本。")
  ...
  match_key = report_match_key(kind, indicator_ids, conditions, config.sample_identity)
  return {
    "report_id": f"{kind}-{indicator_ids[0]}"
    if kind == "single"
