...
安全解包在代码根目录保留 `.trainer-source.json`（原始已验证清单）。Research 制品记录优先使用此提交证据，并复核清单文件的大小/哈希及额外文件；文件变化标记 `dirty`，损坏或不可读证据拒绝，不回退到父目录 Git。源码变更检查忽略解释器生成的 `__pycache__`，以及从完整已核验 pyproject 推导出的本项目标准 `.egg-info` 元数据文件；不会忽略整个元数据目录，额外代码、嵌套文件、链接和其他项目元数据仍标记变更。Windows ACL 预检仍检查这些文件的写权限。此记录不代替启动 ACL 检查，也不授权覆盖运行目录或修改依赖。DEVELOPMENT/FINAL_EVALUATION 主路径在读取数据前写入 `source-evidence.json`，纳入结果及开发锁哈希；部署包非干净状态或证据损坏时失败。最终评估要求父开发锁包含该文件，任一侧来自部署包时，双方源码证据必须完全一致，否则在加载父模型前失败。普通 Git 开发目录记录其状态用于审计，不把开发工作区视作已冻结环境。
