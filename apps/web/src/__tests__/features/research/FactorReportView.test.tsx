...
import {
  factorGroupLabel,
  formatResearchRate,
} from '@/features/research/factorPresentation';
import type { FactorReportQuery } from '@/generated/gql/graphql';

export const factorReportFixture: NonNullable<
  FactorReportQuery['factorReport']
> = {
  reference: {
    runKey: 'a'.repeat(64),
    reportId: 'joint-1',
    kind: 'joint',
    factorIds: ['change_pct', 'volume_ratio'],
    studyId: 'factor-study',
    version: 'v1',
    runId: 'run-1',
...
describe('FactorReportView', () => {
  ...
  it('shows independent horizon counts, uncertainty and no predicted stock probability', () => {
    render(<FactorReportView report={factorReportFixture} />);
  ...
  it('does not convert missing or zero estimates into optimistic values', () => {
    ...
    expect(
      factorGroupLabel(
        'condition_1',
        [{ factor_id: 'rsi12', operator: 'lte', value: 30 }],
