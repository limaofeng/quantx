...
import { FactorReportDrawer } from '@/features/screening/components/FactorReportDrawer';
import type { StockFactorReportMatchesQuery } from '@/generated/gql/graphql';

vi.mock('@/features/screening/hooks/useFactorResearch', () => ({
  useFactorReport: () => ({ detail: null, fetching: false, refresh: vi.fn() }),
}));

const referenceMatch: StockFactorReportMatchesQuery['stockFactorReportMatches'][number] =
  {
    requestId: 'joint',
    ...
    reports: [],
    configJson: {},
    command: 'uv run quantx-research run --config factor-study.json',
