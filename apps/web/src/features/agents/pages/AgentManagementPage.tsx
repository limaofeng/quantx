...
function statusAppearance(status: string) {
  ...
  if (normalized === 'RECONCILING' || normalized === 'RECONCILE_REQUIRED') {
    return {
      label: '对账中',
      icon: RefreshCw,
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-2 text-xs text-slate-500">{device.id}</p>
                      <p className="mt-3 text-sm text-slate-400">
                        账户：{device.authorizedAccountIds.join('、') || '无'}
                      </p>
                      <p className="mt-2 text-xs text-slate-500">
                        最后在线：{formatLastSeen(device.lastSeenAt)}
                      </p>
                      {device.capabilities.length > 0 && (
                        <div className="mt-3 flex flex-wrap gap-1.5">
                          {device.capabilities.map(capability => (
                            <span
                              key={capability}
                              className="rounded-md border border-white/10 bg-white/5 px-2 py-1 text-[11px] text-slate-400"
                            >
                              {capability}
                            </span>
                          ))}
