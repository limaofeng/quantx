    ...
    >
      <span className="flex items-start justify-between gap-2">
        <span className="min-w-0">
          ...
          <span className="mt-0.5 block truncate font-mono text-ui-micro text-slate-400">
      ...
      <span
        className={`mt-2 block font-mono text-ui-display font-black leading-none tabular-nums ${quoteTone(movement)}`}
      >
        {formatMarketPrice(quote?.currentPrice)}
      </span>
      <span className="mt-1.5 flex items-center gap-2 text-ui-caption font-black tabular-nums">
        <span className={`font-mono ${quoteTone(quote?.change)}`}>
...
function Metric({
  ...
  return (
    <div className="min-w-0">
      <div className="text-ui-micro font-bold uppercase tracking-wider text-slate-600">
        {label}
      </div>
      <div
        className={`mt-1 truncate font-mono text-ui-label font-bold tabular-nums ${tone}`}
      >
        {value}
      </div>
    </div>
...
function ComparisonCard({
  ...
  return (
    <div className="rounded-lg border border-white/5 bg-black/10 p-3 xl:p-2">
      <div className="text-ui-caption font-bold text-slate-500">{label}</div>
      <div
        className={`mt-2 font-mono text-ui-heading font-black xl:mt-1 xl:text-ui-title ${tone}`}
      >
        {value}
      </div>
      <div className="mt-1 truncate text-ui-micro text-slate-600 xl:mt-0.5">
        {detail}
      </div>
    </div>
  );
...
function EmptyState({ children }: { children: string }) {
  return (
    <div className="flex min-h-36 items-center justify-center rounded-lg border border-dashed border-white/10 bg-black/10 px-ui-section text-center text-ui-caption leading-5 text-slate-600 xl:min-h-24 xl:px-3">
      {children}
    </div>
  );
}
      ...
      {industries.slice(0, 6).map(industry => (
        ...
        >
          <div className="flex items-center justify-between gap-2">
            <span className="truncate text-ui-caption font-black text-slate-200">
              {industry.industry}
            </span>
            <span className="font-mono text-ui-label font-black text-rose-400">
          ...
          <div className="mt-2 flex items-center gap-2 text-ui-micro text-slate-600">
      ...
      {candidates.slice(0, 6).map(candidate => (
        ...
        >
          <span className="flex min-w-0 items-center gap-2.5">
            <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-rose-500/10 font-mono text-ui-caption font-black text-rose-300">
              {candidate.radarScore.toFixed(0)}
            </span>
            <span className="min-w-0">
              <span className="block truncate text-ui-caption font-bold text-slate-200 group-hover:text-white">
                {candidate.name}
              </span>
          ...
          <span className="shrink-0 text-right">
            <span className="block text-ui-caption font-black text-rose-400">
      ...
      {items.slice(0, 6).map(item => (
        ...
        >
          ...
          <span className="text-right">
            <span className="block font-mono text-ui-caption font-black text-cyan-300">
              {formatRatio(item.volumePaceRatio)}
            </span>
            <span className="block text-ui-micro text-slate-600">量速</span>
          </span>
          <span className="w-14 text-right">
            <span
              className={`block font-mono text-ui-caption font-black ${quoteTone(item.changePct)}`}
            >
...
function StockRankingList({
  ...
  return (
    ...
    >
      <div className="grid grid-cols-4 items-center gap-1.5 border-b border-white/5 bg-black/10 px-2 py-1.5 text-ui-micro font-bold uppercase tracking-wider text-slate-600 sm:grid-cols-5 sm:gap-2 sm:px-3">
        <span />
        <h3
          className="flex min-w-0 items-center gap-1.5 text-ui-caption font-black normal-case tracking-normal text-slate-300"
          id={headingId}
        >
          <DirectionIcon
        ...
        <ol className="divide-y divide-white/5">
          {visibleRows.map((row, index) => (
            <li key={row.code}>
              ...
              >
                <span
                  className={`font-mono text-ui-micro font-black ${
                ...
                <span className="flex min-w-0 items-baseline gap-2">
                  <span className="truncate text-ui-caption font-bold text-slate-200 transition-colors duration-200 group-hover:text-white">
                    {row.name}
                  </span>
                  <span className="hidden shrink-0 font-mono text-ui-micro text-slate-600 sm:inline">
                    {row.code}
                  </span>
                </span>
                <span className="text-right font-mono text-ui-caption font-bold tabular-nums text-slate-300">
                  {formatMarketPrice(row.currentPrice)}
                </span>
                <span
                  className={`text-right font-mono text-ui-caption font-black tabular-nums ${quoteTone(row.changePct)}`}
                >
                  {formatMarketPercent(row.changePct)}
                </span>
                <span className="hidden text-right font-mono text-ui-micro tabular-nums text-slate-500 sm:block">
    ...
    <MarketStudioShell
      content={
        <div className="studio-workspace-surface flex min-h-0 flex-1 flex-col overflow-hidden">
          <header className="studio-workspace-surface flex shrink-0 flex-col gap-2 border-b border-white/5 px-ui-section py-2 xl:flex-row xl:items-center xl:justify-between xl:gap-3 xl:py-1">
            <div className="flex min-w-0 items-center">
              <div className="min-w-0">
                <div className="truncate text-ui-label font-black uppercase tracking-[0.18em] text-slate-100">
                  行情工作台
                </div>
            ...
            <div className="flex w-full shrink-0 items-center justify-end gap-2 xl:w-auto">
              <div className="min-w-0 flex-1 text-right sm:flex-none">
                <div
                  className={`text-ui-caption font-bold ${
                ...
                <div className="mt-0.5 font-mono text-ui-micro text-slate-600">
                  {marketDataLabel}
                </div>
              </div>
              <Button
                aria-label="刷新全部行情"
                className="h-control-compact w-8 rounded-lg border border-white/10 bg-white/[0.03] text-slate-400 hover:border-white/20 hover:text-slate-100"
                onClick={refreshAll}
                size="icon"
                variant="ghost"
              >
          ...
          <main className="min-h-0 flex-1 overflow-x-hidden overflow-y-auto scroll-smooth">
            <div className="studio-content-width mx-auto px-3 pb-10 pt-3 sm:px-ui-section lg:px-ui-section xl:px-3 xl:pt-2">
              <nav
                aria-label="行情工作台分区"
              ...
              >
                {sectionNavigation.map(({ icon: Icon, id, label }) => (
                  <button
                    key={id}
                    className="flex min-w-max cursor-pointer items-center gap-1.5 rounded-md px-3 py-2 text-ui-caption font-bold text-slate-500 transition-colors hover:bg-white/[0.04] hover:text-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500/70 xl:py-1.5"
                    onClick={() => scrollTo(id)}
                    type="button"
                  >
              ...
              <section className="scroll-mt-14" id="market-overview">
                <div className="overflow-hidden rounded-panel border border-white/10 bg-slate-900">
                  <div className="flex flex-col justify-between gap-3 border-b border-white/5 px-ui-section py-3 sm:flex-row sm:items-center xl:px-3 xl:py-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h1 className="text-ui-body font-black text-slate-100">
                            {session.label}
                          </h1>
                          <span
                            className={`rounded border px-1.5 py-0.5 text-ui-micro font-black ${toneClasses[market.summary.tone]}`}
                          >
                            {market.summary.toneLabel}
                          </span>
                          {marketDataIsStale ? (
                            <span className="rounded border border-amber-400/25 bg-amber-400/10 px-1.5 py-0.5 text-ui-micro font-black text-amber-300">
                              数据滞后
                            </span>
                          ) : null}
                          {pulseDataIsMissing ? (
                    ...
                    <div className="flex items-center gap-ui-section">
                      <div className="text-right">
                        <div
                          className={`font-mono text-ui-page-title font-black ${quoteTone(market.summary.averageChange)}`}
                        >
                      ...
                      <div className="text-right">
                        <div className="font-mono text-ui-page-title font-black text-slate-100">
                          {breadthTotal
                  ...
                  >
                    {market.indices.length > 0 ? (
                      ...
                      <div className="flex min-h-28 w-64 shrink-0 items-center rounded-lg border border-dashed border-white/10 px-ui-section text-ui-label text-slate-600">
                    ...
                    <Link
                      ...
                      href="/market/indices"
                    >
                      <span className="flex flex-col text-ui-caption font-black leading-[1.15] text-slate-200">
                        <span>全部</span>
                        <span>指数</span>
                      </span>
                  ...
                  <div className="border-t border-white/5 p-ui-section xl:p-3">
                      ...
                      <div className="xl:flex xl:flex-col xl:justify-center xl:px-1">
                        <div className="flex items-end justify-between gap-3">
                          <div>
                            ...
                            <div className="mt-1 text-ui-micro text-slate-600 xl:mt-0.5">
                              非 ST 股票 · {pulseSnapshotLabel}
                            </div>
                          </div>
                          <div className="flex items-center gap-ui-section font-mono text-ui-caption font-black xl:gap-2.5">
                ...
                <div className="mt-3 grid gap-3 xl:mt-2 xl:grid-cols-2 xl:gap-2">
                  <section className="overflow-hidden rounded-panel border border-white/10 bg-slate-900">
                    {selected ? (
                      <>
                        <div className="flex flex-col justify-between gap-ui-section border-b border-white/5 px-ui-section py-3 sm:flex-row sm:items-end xl:px-3 xl:py-2">
                          <div>
                            <div className="flex items-center gap-2">
                            ...
                            <div className="mt-2 flex items-baseline gap-3">
                              <span className="font-mono text-ui-display font-black text-slate-100">
                                {formatMarketPrice(selectedQuote?.currentPrice)}
                              </span>
                              <span
                                className={`font-mono text-ui-body font-black ${quoteTone(selectedQuote?.changePercent)}`}
                              >
                                {formatMarketPercent(
                          ...
                          <div className="grid grid-cols-4 gap-ui-section xl:gap-3">
                            <Metric
                              label="开盘"
                        ...
                        <div className="px-2 pb-2 pt-1">
                          <div className="flex items-center justify-between px-2 py-1 text-ui-micro font-bold uppercase tracking-wider text-slate-600">
                            <span>分钟走势</span>
                  ...
                  <section className="rounded-panel border border-white/10 bg-slate-900 p-ui-section xl:p-3">
                      ...
                      {rankedIndices.map((item, position) => (
                        ...
                        >
                          <span className="flex min-w-0 items-center gap-2">
                            <span className="w-4 shrink-0 font-mono text-ui-micro font-bold text-slate-700">
                              {position + 1}
                            </span>
                          ...
                          <span
                            className={`shrink-0 font-mono text-ui-caption font-black ${quoteTone(item.quote?.changePercent)}`}
                          >
              ...
              >
                <div className="mb-2 flex items-end justify-between gap-3 px-1">
                  ...
                  <Link
                    className="hidden items-center gap-1 text-ui-caption font-bold text-slate-500 hover:text-slate-200 sm:flex"
                    href="/limit-up-board"
                  >
                ...
                <div className="grid gap-3 xl:grid-cols-3 xl:gap-2">
                  <section className="rounded-panel border border-white/10 bg-slate-900 p-ui-section xl:p-3">
                    <div className="mb-3 flex items-start justify-between gap-3 xl:mb-2">
                      ...
                      <span
                        className={`inline-flex items-center gap-1 rounded-md border px-2 py-1 text-ui-micro font-black ${
                          radar.isScannerRunning
                  ...
                  <section className="rounded-panel border border-white/10 bg-slate-900 p-ui-section xl:p-3">
                    <div className="mb-1 flex items-start justify-between gap-3">
                      ...
                      <span className="font-mono text-ui-caption font-black text-rose-300">
                  ...
                  <section className="rounded-panel border border-white/10 bg-slate-900 p-ui-section xl:p-3">
                    <div className="mb-1 flex items-start justify-between gap-3">
                      ...
                      <span
                        className={`inline-flex items-center gap-1 text-ui-micro font-bold ${
                ...
                <div className="overflow-hidden rounded-panel border border-white/10 bg-slate-900">
                  <div className="flex items-center justify-between gap-3 border-b border-white/5 px-3 py-2">
                    <div className="flex min-w-0 items-center gap-2">
                      ...
                      <h2 className="text-ui-label font-black text-slate-100">
                        股票排行
                      </h2>
                      <span className="truncate text-ui-micro text-slate-600">
                        {pulseSnapshotLabel} · 非 ST
                      </span>
                    </div>
                    <span className="shrink-0 text-ui-micro font-bold text-slate-600">
                      涨跌各 8 只
                    </span>
                  </div>
                  <div
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-6">
                  {quickTools.map(tool => {
                    ...
                    return (
                      <Link
                        key={tool.label}
                        className="group cursor-pointer rounded-panel border border-white/5 bg-slate-900 p-3.5 transition-colors hover:border-white/15 hover:bg-white/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500/70 xl:p-2.5"
                        href={tool.href}
                      >
                        ...
                        <span className="mt-3 block text-ui-caption font-black text-slate-200">
                          {tool.label}
                        </span>
                        <span className="mt-1 block truncate text-ui-micro text-slate-600">
                          {tool.description}
                        </span>
                      </Link>
                    );
