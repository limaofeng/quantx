...
import type { FactorReportQuery } from '@/generated/gql/graphql';
import { financialToneClass } from '@/shared/utils/financialColors';

import { factorGroupLabel, formatResearchRate } from '../factorPresentation';

type Report = NonNullable<FactorReportQuery['factorReport']>;
    ...
    <div className="space-y-3">
      {definitions.success && (
        <section className="rounded-md border border-white/10 p-3">
          <h2 className="mb-2 text-ui-title font-medium">因子定义与计算口径</h2>
          {definitions.data.map(factor => (
            <div key={factor.id} className="mb-2">
              <h3 className="text-ui-label text-slate-200">{factor.label}</h3>
      ...
      {report.configJson != null && (
        <Button
          ...
          onClick={() => {
            ...
            link.href = url;
            link.download = 'factor-study-resolved.json';
            link.click();
...
export function FactorReportView({
  report,
  compact = false,
}: {
  report: Report;
  ...
  return (
    <div className="space-y-3 text-ui-body text-slate-200">
      ...
      <div className="flex flex-wrap gap-2 text-ui-caption text-slate-400">
        ...
        <span>因子版本 {report.factorVersion}</span>
      ...
      <div
        ...
        role="img"
        aria-label={`${factorGroupLabel(group, report.conditions, report.definitions)}未来各交易日日期等权上涨比例与基准曲线`}
      >
