...
import { useFactorReport } from '@/features/screening/hooks/useFactorResearch';
...
export function FactorStudyRun({
  run,
}: {
  ...
  const reference = selected
    ? run.factorReports.find(item => item.reportId === selected)
    : run.factorReports[0];
  const { detail, error, fetching, refresh } = useFactorReport(
    reference?.runKey ?? '',
    reference?.reportId ?? ''
  );
  return (
    <main className="studio-workspace-surface h-full overflow-y-auto text-slate-200">
      <header className="sticky top-0 z-20 flex flex-wrap items-center gap-3 border-b border-white/10 bg-background p-ui-section">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="text-ui-title font-semibold">因子历史研究</h1>
      ...
      <div className="studio-content-width mx-auto space-y-3 p-ui-section">
        ...
        {reference ? (
          <Select value={reference.reportId} onValueChange={setSelected}>
            <SelectTrigger aria-label="选择因子报告">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {run.factorReports.map(item => (
                <SelectItem key={item.reportId} value={item.reportId}>
                  {item.kind === 'joint' ? '条件交集报告' : '单因子'} ·{' '}
                  {item.factorIds.join(' + ')}
                </SelectItem>
              ))}
            </SelectContent>
          ...
          >
            {selected
              ? '链接中的因子报告不存在，未替换为其他报告。'
              : '本次运行没有可用因子报告。请检查离线运行状态与产物。'}
            {selected && (
              <Button
                size="sm"
                variant="outline"
        ...
        {detail && (
          <FactorReportView key={reference?.reportId} report={detail} />
        )}
      </div>
    </main>
  );
