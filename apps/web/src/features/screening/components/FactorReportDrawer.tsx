...
import { FactorReportView } from '@/features/research/components/FactorReportView';
import { buildResearchRunPath } from '@/features/research/model';
import type { StockFactorReportMatchesQuery } from '@/generated/gql/graphql';

import {
  describeFactorCondition,
  validateFactorConditions,
} from '../factorModel';
import { useFactorReport } from '../hooks/useFactorResearch';
import type { FactorDefinition, ScreeningCriteria } from '../types';

type Match = StockFactorReportMatchesQuery['stockFactorReportMatches'][number];
...
function downloadConfig(config: unknown) {
  ...
  link.href = url;
  link.download = 'factor-study.json';
  link.click();
  URL.revokeObjectURL(url);
}

export function FactorReportDrawer({
  focus,
  onClose,
  criteria,
  factors,
  ...
  const {
    ...
    error: detailError,
    refresh,
  } = useFactorReport(reference?.runKey ?? '', reference?.reportId ?? '');
  const factor = factors.find(item => `single:${item.id}` === focus);
  const conditionError = validateFactorConditions(
      ...
      >
        <SheetHeader className="shrink-0 pr-8">
          <SheetTitle>
            {focus === 'joint'
              ? new Set(
                  (criteria.factorConditions ?? []).map(
                    condition => condition.factorId
                  )
                ).size >= 2
                ? '当前组合研究报告'
                : '当前条件研究报告'
              : `${factor?.label ?? '因子'} · 单因子报告`}
          </SheetTitle>
          <SheetDescription>
            只读历史研究 ·{' '}
          ...
          {factor && (
            <div className="rounded-md border border-white/10 p-3 text-ui-label text-slate-300">
              ...
              <p className="mt-1 font-mono text-ui-caption text-slate-400">
                {factor.id} · {factor.version} · {factor.lookback} 日 ·{' '}
                {factor.unit}
              </p>
              {!factor.researchSupported && (
                <p className="mt-2 text-amber-200">
                  {factor.unsupportedReason ??
                    '历史研究暂未覆盖，选股仍可使用。'}
                </p>
              )}
              <p className="mt-2 text-slate-400">
                单因子报告展示总体分组，不代表当前任意阈值的精确研究。
              </p>
            </div>
          )}
          {focus === 'joint' && (
            <div className="rounded-md border border-white/10 p-3 text-ui-label text-slate-300">
              {(criteria.factorConditions ?? []).map((condition, index) => (
                <p key={index}>{describeFactorCondition(condition, factors)}</p>
              ))}
            </div>
          )}
