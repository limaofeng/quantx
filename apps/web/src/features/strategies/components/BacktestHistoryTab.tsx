...
type BacktestMenuPayload =
  { kind: 'template' } | { backtest: StrategyBacktest; kind: 'backtest' };
...
function optionalMetrics(
  ...
  const entries = Object.entries(record).filter(
    (entry): entry is [string, number | string | null | undefined] =>
      entry[1] === undefined ||
      entry[1] === null ||
...
export default function BacktestHistoryTab({
  ...
  const backtests: StrategyBacktest[] = (
    (data?.backtestHistory || []) as unknown[]
  ).map(raw => {
    ...
    return {
      ...
      parameters: optionalRecord(record.parameters),
      instruments: optionalStringArray(record.instruments),
      backtestStartTime: optionalString(
        record.backtestStartTime,
        record.backtest_start_time
      ),
      backtestEndTime: optionalString(
        record.backtestEndTime,
        record.backtest_end_time
      ),
      startTime: optionalString(record.startTime, record.start_time),
      endTime: optionalString(record.endTime, record.end_time),
      metrics: optionalMetrics(record.metrics),
      status: String(record.status || 'PENDING'),
      errorMessage: optionalString(record.errorMessage, record.error_message),
      resultPath: optionalString(record.resultPath, record.result_path),
  ...
  const confirmDeleteVersion = async () => {
    ...
    try {
      ...
      const payload = result.data?.deleteBacktestVersion;
      if (!payload?.success) {
