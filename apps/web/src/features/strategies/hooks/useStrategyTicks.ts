...
export function useStrategyTicks(
  ...
  const [{ data, error, fetching }] = useSubscription({
    query: STRATEGY_TICKS_SUBSCRIPTION,
  ...
  useEffect(() => {
    if (data?.strategyTicks) {
      const newTick: StrategyTickData = {
        ...
        bidPrice: data.strategyTicks.bidPrice ?? undefined,
        askPrice: data.strategyTicks.askPrice ?? undefined,
        bidVolume: data.strategyTicks.bidVolume ?? undefined,
        askVolume: data.strategyTicks.askVolume ?? undefined,
        openPrice: data.strategyTicks.openPrice ?? undefined,
        highPrice: data.strategyTicks.highPrice ?? undefined,
        lowPrice: data.strategyTicks.lowPrice ?? undefined,
        preClose: data.strategyTicks.preClose ?? undefined,
