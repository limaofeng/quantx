...
import {
  ...
  StrategiesDocument,
  type Strategy,
  type StrategyInstanceCreateInput,
...
export default function StrategyRunPage() {
  ...
  const [
    {
      ...
      fetching: currentAccountLoading,
      error: currentAccountError,
    },
  ] = useQuery({
    query: GetCurrentAccountDocument,
    pause: runMode !== StrategyRunMode.Live,
  ...
  const currentAccountId = currentAccountData?.currentAccount?.id || '';

  useEffect(() => {
    if (runMode !== StrategyRunMode.Live || !currentAccountId) return;
    setStrategyConfig(previous =>
      previous.account_id === currentAccountId
        ? previous
        : { ...previous, account_id: currentAccountId }
    );
  }, [currentAccountId, runMode]);
  ...
  const saveStrategy = async (autoStart: boolean) => {
    ...
    if (runMode === StrategyRunMode.Live && !currentAccountId) {
      setFormError(
        currentAccountError
          ? `交易账户加载失败：${currentAccountError.message}`
          : '当前没有可用交易账户，不能创建实盘实例。'
      );
      return;
    }
    ...
    try {
      ...
      const input: StrategyInstanceCreateInput = {
      ...
      if (mode === StrategyRunMode.Backtest) {
        ...
        if (dateRange.to.getTime() < dateRange.from.getTime()) {
          throw new Error('回测结束日期不能早于开始日期');
        }
        input.startTime = toBacktestBoundaryIso(dateRange.from, 'start');
        input.endTime = toBacktestBoundaryIso(dateRange.to, 'end');
      }
      ...
      const createdId = result.data?.createStrategyInstance?.id;
      if (!createdId) {
        throw new Error('策略实例创建成功但未返回实例 ID');
      }
      ...
      setFormError(null);
      setLocation(
        `/strategies/${strategy.id}/runs/${encodeURIComponent(createdId)}`
      );
    } catch (e) {
      logger.error('Failed to save strategy:', { error: e });
      setFormError(
        e instanceof Error ? e.message : '策略实例创建失败，请稍后重试。'
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleSaveAndRun = async () => {
    // Live instances are always created stopped. Starting live trading is a
    // separate, explicitly confirmed action on the instance detail page.
    await saveStrategy(runMode !== StrategyRunMode.Live);
  };
        ...
        <div className="grid grid-cols-3 gap-1 rounded-md border border-white/10 bg-white/[0.03] p-1">
          {[
            ...
            <button
              ...
              className={cn(
                'h-7 rounded text-ui-caption font-black uppercase tracking-wider transition-colors',
                runMode === item.mode
  ...
  const createContent = (
    <div className="studio-workspace-surface h-full overflow-y-auto p-3 custom-scrollbar">
      <div className="mx-auto max-w-6xl space-y-ui-section pb-12">
        {/* Premium Back & Header */}
        <div className="flex flex-col gap-ui-panel">
          {/* Unified Header Card */}
          <div className="rounded-panel bg-[#0F1729] border border-white/5 p-ui-section relative overflow-hidden shadow-none">
            <ProfessionalBackground />

            <div className="relative z-10 flex flex-col gap-ui-panel">
              {/* Top Row: Meta & Switcher */}
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Button
                    variant="ghost"
                    className="group h-control-compact px-3 -ml-3 rounded-full text-ui-caption font-bold text-slate-500 hover:text-red-400 hover:bg-red-500/10 uppercase tracking-wider gap-2 transition-all duration-300"
                    onClick={() => setLocation('/strategies')}
                  >
                ...
                <Popover open={openSelector} onOpenChange={setOpenSelector}>
                  <PopoverTrigger asChild>
                    <Button
                      variant="ghost"
                      className="h-control-compact px-3 text-ui-caption font-bold text-slate-500 hover:text-white hover:bg-white/5 rounded-full uppercase tracking-wider gap-2 border border-transparent hover:border-white/10 transition-all shadow-sm"
                    >
                  ...
                  <PopoverContent
                    align="end"
                    className="w-[300px] p-0 bg-[#0B1121] border-red-900/30 rounded-panel overflow-hidden shadow-none shadow-black/50"
                  >
                    <Command className="bg-transparent text-slate-200">
                      <CommandInput
                        placeholder="搜索策略..."
                        className="h-12 border-b border-white/5 text-ui-label font-bold"
                      />
                      <CommandEmpty className="py-ui-panel text-center text-ui-label text-slate-500 font-bold uppercase tracking-wider">
                        暂无相关策略
                      </CommandEmpty>
                        ...
                        <CommandGroup>
                          {availableStrategies?.map(s => (
                            <CommandItem
                              ...
                              className="text-ui-label font-bold py-3 px-3 rounded-lg aria-selected:bg-red-600/10 aria-selected:text-red-400 cursor-pointer"
                            >
                              <Check
                                className={cn(
                                  'mr-2 h-3 w-3',
                              ...
                              <div className="flex flex-col">
                                <span>{s.name}</span>
                                <span className="text-ui-caption text-slate-500 font-normal mt-0.5">
                                  {s.category}
                                </span>
                              </div>
                            </CommandItem>
              ...
              {strategy ? (
                /* Content Grid */
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-ui-panel lg:gap-ui-panel items-start">
                  {/* Left: Strategy Info (7 cols) */}
                  <div className="lg:col-span-7 space-y-ui-section h-full flex flex-col justify-between">
                    <div className="space-y-3">
                      <div className="flex items-center gap-2">
                        <div className="w-1.5 h-1.5 rounded-full bg-red-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]" />
                        <span className="text-ui-caption font-bold uppercase tracking-[0.2em] text-red-400">
                          策略配置
                        </span>
                      </div>
                      <h1 className="text-ui-display-lg md:text-4xl font-black text-white uppercase tracking-tight leading-none italic">
                        {strategy.name}{' '}
                        <span className="text-slate-500 not-italic">策略</span>
                      </h1>
                      <p className="text-ui-label text-slate-400 font-medium leading-relaxed max-w-xl border-l-2 border-slate-800 pl-3">
                        {strategy.description}
                      </p>
                    </div>

                    <div className="flex flex-wrap gap-3">
                      <div className="flex items-center gap-2 px-ui-section py-2 rounded-panel bg-slate-800/50 border border-slate-700/50">
                        <Shield size={14} className="text-red-400" />
                        <span className="text-ui-label font-bold text-slate-300 uppercase tracking-wider">
                          {getRiskLevelName(
                  ...
                  <div className="lg:col-span-4 space-y-ui-section">
                    <div className="space-y-2">
                      <Label className="text-ui-caption font-bold text-slate-500 uppercase tracking-wider">
                        策略实例名称
                      </Label>
                      <div className="relative group">
                        <Input
                          ...
                          className="h-[40px] bg-transparent border-0 border-b border-white/10 rounded-none px-0 text-ui-heading font-bold text-white placeholder:text-slate-600 focus-visible:ring-0 focus-visible:border-red-500 transition-all py-2"
                          placeholder="为本次运行命名..."
                        />
                        ...
                        {[
                          {
                            ...
                            icon: LineChart,
                            activeClass:
                              'border-red-400 bg-red-500 shadow-none',
                          },
                          {
                            ...
                            icon: TestTube,
                            activeClass:
                              'border-emerald-400 bg-emerald-500 shadow-none',
                          },
                          {
                            mode: StrategyRunMode.Live,
                            label: '实盘',
                            icon: Rocket,
                            activeClass:
                              'border-rose-400 bg-rose-500 shadow-none',
                          },
                          ...
                          <button
                            ...
                            className={cn(
                              'cursor-pointer group relative overflow-hidden rounded-panel border p-3.5 flex flex-col items-center justify-center gap-2 transition-all duration-300',
                              runMode === mode
                                ? activeClass
                          ...
                          >
                            ...
                            <div className="relative z-10 text-center">
                              <span
                                className={cn(
                                  'block text-ui-micro font-black uppercase tracking-[0.15em] transition-colors duration-300',
                                  runMode === mode
                                    ? 'text-white'
                                    : 'text-slate-500 group-hover:text-slate-300'
                                )}
                    ...
                    {runMode === StrategyRunMode.Backtest && (
                      <div className="mt-4 animate-in fade-in slide-in-from-top-2 duration-300 space-y-2">
                        ...
                        <DateRangePicker
                          ...
                          buttonClassName="h-[40px] bg-slate-900/50 border-white/10 text-white font-mono text-ui-label focus-visible:ring-1 focus-visible:ring-red-500 rounded-panel px-3"
                        />
                      </div>
                    )}

                    {runMode === StrategyRunMode.Live && (
                      <div className="rounded-panel border border-rose-500/20 bg-rose-500/10 px-3 py-2 text-ui-caption leading-relaxed text-rose-200">
                        {currentAccountLoading
                          ? '正在校验当前交易账户…'
                          : currentAccountId
                            ? `交易账户：${currentAccountId}。实例只创建、不自动启动，需在详情页再次确认。`
                            : '没有可用交易账户，实盘创建已禁用。'}
                      </div>
                    )}

                    {formError && (
                      <div className="flex items-start gap-2 rounded-panel border border-rose-500/20 bg-rose-500/10 px-3 py-2 text-ui-caption font-bold leading-relaxed text-rose-300">
                ...
                /* Empty State */
                <div className="py-20 text-center space-y-ui-section">
                  ...
                  <p className="text-ui-body text-slate-400 font-medium max-w-lg mx-auto leading-relaxed">
              ...
              {ConfigPanel && (
                <ConfigPanel
                  ...
                  submitDisabled={
                    submitting ||
                    (runMode === StrategyRunMode.Live &&
                      (currentAccountLoading || !currentAccountId))
                  }
                  submitLabel={
                    runMode === StrategyRunMode.Live
                      ? '创建实盘实例（不启动）'
        ...
        <div className="flex h-10 shrink-0 items-center justify-between border-b border-white/5 bg-[#0b1120]/80 px-3">
          ...
          <div className="flex shrink-0 items-center gap-2">
            <button
              ...
              className="h-7 rounded-md border border-white/10 px-2.5 text-ui-caption font-bold uppercase tracking-wider text-slate-400 transition-colors hover:border-red-500/40 hover:text-red-300"
            >
              保存
            </button>
            <button
              ...
              className="h-7 rounded-md bg-red-500 px-2.5 text-ui-caption font-black uppercase tracking-wider text-white transition-colors hover:bg-red-400"
            >
              保存并运行
            </button>
          </div>
