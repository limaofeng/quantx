        ...
        <div className="grid grid-cols-1 gap-ui-section xl:grid-cols-[minmax(0,1fr)_420px]">
          <div className="space-y-ui-section">
            <section className="rounded-panel border border-slate-200/60 bg-white/70 p-ui-section shadow-sm backdrop-blur-sm dark:border-white/5 dark:bg-white/[0.03]">
                ...
                <div className="flex rounded-lg border border-slate-200/60 bg-slate-50 p-1 dark:border-white/5 dark:bg-white/[0.03]">
                  {[
                    ...
                    <button
                      ...
                      className={cn(
                        'h-7 rounded-md px-3 text-ui-caption font-black transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/60',
              ...
              {targetMode === 'auto' ? (
                <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
                  <div className="rounded-panel border border-violet-500/20 bg-violet-500/10 p-ui-section">
                    <div className="flex items-center gap-2 text-ui-body font-black text-violet-700 dark:text-violet-300">
                      <Users className="h-4 w-4" />
                      持仓股与自选股
                    </div>
                    <p className="mt-2 text-ui-label font-medium leading-relaxed text-slate-600 dark:text-slate-400">
                      Flow 会调用
                      collect_disclosure_sync_symbols，自动合并需要跟踪的标的集合。
                    </p>
                  </div>
                  <div className="rounded-panel border border-slate-200/60 bg-slate-50/80 p-ui-section dark:border-white/5 dark:bg-white/[0.02]">
                    <div className="flex items-center gap-2 text-ui-body font-black text-slate-800 dark:text-slate-100">
            ...
            <section className="rounded-panel border border-slate-200/60 bg-white/70 p-ui-section shadow-sm backdrop-blur-sm dark:border-white/5 dark:bg-white/[0.03]">
              ...
              <div className="mt-4 rounded-lg border border-violet-500/15 bg-violet-500/5 p-3 text-ui-label font-medium leading-relaxed text-violet-700 dark:text-violet-300">
            ...
            <section className="rounded-panel border border-slate-200/60 bg-white/70 p-ui-section shadow-sm backdrop-blur-sm dark:border-white/5 dark:bg-white/[0.03]">
              ...
              <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
                {[
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className="flex h-5 w-5 items-center justify-center rounded bg-violet-500/10 font-mono text-ui-caption font-black text-violet-600">
                        {step}
                      </span>
                      <span className="text-ui-label font-black text-slate-900 dark:text-white">
                        {title}
                      </span>
                    </div>
                    <p className="mt-2 text-ui-caption font-medium leading-relaxed text-slate-500">
                      {description}
                    </p>
                  </div>
                ))}
