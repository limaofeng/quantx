...
export function ReverseRepoDataPage() {
  ...
  const [{ data }] = useQuery({
    query: GET_REVERSE_REPO_INSTRUMENTS,
  });
  ...
  const parsedData = useMemo(() => {
    ...
    return data.instruments
      .map(item => {
        const name = item.name || item.instrumentId;
        const durationMatch = name.match(/\d+/);
  ...
  const filteredData = useMemo(
    () => parsedData.filter(item => item.type === activeMarket),
  ...
  const bestRates = useMemo(() => {
    const getBest = (days: number, mkt: string) => {
      const items = parsedData.filter(
        i => i.duration === days.toString() && i.type === mkt
      );
      if (!items.length) return { rate: '0.000', rawChange: 0 };
      return items.reduce(
        (previous, current) =>
          parseFloat(current.rate) > parseFloat(previous.rate)
            ? current
            : previous,
        items[0]
      );
  ...
  return (
    <DataStudioPageFrame
      ...
      title="逆回购数据"
    >
      <div className="flex flex-col gap-ui-section animate-fade-in pb-6">
        ...
        <div className="flex items-center justify-between gap-ui-section">
          <div className="flex items-center gap-3">
            <Button
              variant="ghost"
              size="icon"
              className="h-control-compact w-8 rounded-lg bg-white/50 dark:bg-slate-800/50 hover:bg-white dark:hover:bg-slate-800 border border-slate-200/60 dark:border-slate-700/60 shadow-sm backdrop-blur-sm transition-all"
              onClick={() => setLocation('/settings/data')}
            >
              <ArrowLeft className="w-4 h-4 text-slate-600 dark:text-slate-400" />
            </Button>
            <div>
              <h1 className="text-ui-heading font-black text-slate-800 dark:text-slate-100 tracking-tight leading-none">
                国债逆回购
              </h1>
              <p className="text-ui-caption font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-0.5">
                Cash Management
              </p>
            </div>
          </div>
          ...
          <Card className="relative overflow-hidden border-indigo-100 dark:border-indigo-500/10 shadow-sm bg-gradient-to-br from-indigo-50/50 via-white to-white dark:from-indigo-950/20 dark:via-slate-900/40 dark:to-slate-900/40">
            ...
            <CardContent className="p-3">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-ui-caption font-bold text-indigo-900/60 dark:text-indigo-200/60 uppercase tracking-wider">
                  最优 1天期 (1D)
                </span>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <div className="text-ui-caption text-slate-400 mb-0.5 font-medium">
                    沪市 GC001
                  </div>
                  <div className="text-ui-body font-black text-indigo-600 dark:text-indigo-400 tabular-nums tracking-tight">
                    {bestRates.sh1d.rate}%
                  </div>
                </div>
                <div>
          ...
          <Card className="relative overflow-hidden border-blue-100 dark:border-blue-500/10 shadow-sm bg-gradient-to-br from-blue-50/50 via-white to-white dark:from-blue-950/20 dark:via-slate-900/40 dark:to-slate-900/40">
            ...
            <CardContent className="p-3">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-ui-caption font-bold text-blue-900/60 dark:text-blue-200/60 uppercase tracking-wider">
                  最优 7天期 (7D)
                </span>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <div className="text-ui-body font-black text-blue-600 dark:text-blue-400 tabular-nums tracking-tight">
                    {bestRates.sh7d.rate}%
                  </div>
                </div>
                <div>
            ...
            <CardContent className="p-3 h-full flex items-center justify-between">
              <div className="flex flex-col justify-between h-full">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-ui-caption font-bold text-market-up/70 uppercase tracking-wider">
                    累计收益
                  </span>
                </div>
                <div className="mt-2">
                  <div className="text-ui-display font-black text-market-up tabular-nums tracking-tight">
                    ¥
                    {earnings.total.toLocaleString('en-US', {
                      minimumFractionDigits: 2,
                    })}
                  </div>
                  <div className="text-ui-caption font-medium text-market-up/70 mt-0.5">
              ...
              <div className="flex flex-col gap-2">
                <Button
                  size="sm"
                  className="h-control-compact px-3 text-ui-label bg-primary hover:bg-primary/90 shadow-sm shadow-primary/20"
                >
                  资金详情
                </Button>
              </div>
        ...
        {/* 3. Main Split View */}
        <div className="grid grid-cols-12 gap-ui-section flex-1 min-h-0">
          ...
          <Card className="col-span-12 lg:col-span-8 flex flex-col border-slate-200 dark:border-slate-800 shadow-sm bg-white/80 dark:bg-slate-900/60 backdrop-blur-md">
            ...
            >
              <div className="px-ui-section py-3 flex items-center justify-between border-b border-slate-100 dark:border-slate-800/60">
                <div className="flex items-center gap-ui-section">
                  <h2 className="text-ui-body font-bold text-slate-700 dark:text-slate-200 flex items-center gap-2">
                  ...
                  <TabsList className="h-7 bg-slate-100 dark:bg-slate-800 p-0.5">
                    <TabsTrigger
                      value="SH"
                      className="h-6 text-ui-caption px-3 data-[state=active]:bg-white dark:data-[state=active]:bg-slate-700 shadow-none"
                    >
                      沪市
                    </TabsTrigger>
                    <TabsTrigger
                ...
                <Badge
                  variant="outline"
                  className="text-ui-caption font-mono h-5 px-1.5 text-slate-400 border-slate-200 dark:border-slate-800"
                >
              ...
              <div className="flex-1 overflow-hidden">
                {/* Custom Table Header */}
                <div className="grid grid-cols-12 gap-2 px-ui-section py-2 bg-slate-50/50 dark:bg-slate-900/20 border-b border-slate-100 dark:border-slate-800 text-ui-caption font-bold text-slate-400 uppercase tracking-wider">
                ...
                <ScrollArea className="h-[400px] lg:h-[500px]">
                  <div className="divide-y divide-slate-100 dark:divide-slate-800/60">
                    {filteredData.map(item => (
                      <div
                        key={item.code}
                        className="grid grid-cols-12 gap-2 px-ui-section py-2.5 items-center hover:bg-indigo-50/30 dark:hover:bg-indigo-900/10 transition-colors group cursor-default"
                      >
                        <div className="col-span-2 flex flex-col">
                          <span className="text-ui-label font-bold text-slate-700 dark:text-slate-200 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                            {item.term}
                          </span>
                        ...
                        <div className="col-span-3 text-ui-label font-medium text-slate-600 dark:text-slate-400">
                          {item.name}
                        </div>
                        <div className="col-span-2 text-right font-mono text-ui-body font-bold text-rose-500 tabular-nums bg-rose-50/0 group-hover:bg-rose-50/50 dark:group-hover:bg-rose-900/10 rounded px-1 -mr-1 transition-colors">
                          {item.rate}%
                        </div>
                        <div className="col-span-2 text-right font-mono text-ui-label font-bold text-market-up tabular-nums">
                          {item.earningsPer10k}
                        </div>
                        <div className="col-span-3 text-right flex flex-col items-end">
                          <div className="flex items-center gap-1.5">
                            <Badge
                              variant="secondary"
                              className="h-4 px-1 text-ui-micro min-w-0 bg-slate-100 dark:bg-slate-800 text-slate-500"
                            >
                              {item.days}
                            </Badge>
                            <span className="text-ui-caption text-slate-400 font-medium">
                              {item.arrivalDate}
                            </span>
                          </div>
                        </div>
          ...
          <div className="col-span-12 lg:col-span-4 flex flex-col gap-ui-section">
            {/* Trading Control */}
            <Card className="border-indigo-100 dark:border-indigo-500/10 shadow-sm bg-gradient-to-b from-white to-slate-50/50 dark:from-slate-900 dark:to-slate-900/50">
              <CardHeader className="px-ui-section py-3 border-b border-indigo-50 dark:border-indigo-500/10">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-ui-label font-bold text-indigo-900 dark:text-indigo-100 flex items-center gap-2">
              ...
              <CardContent className="p-ui-section space-y-ui-section">
                <div className="grid grid-cols-2 gap-2">
                  <div className="p-2 rounded-lg bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
                    <div className="text-ui-micro text-slate-400 uppercase tracking-wider mb-1">
                      Max Vol
                    </div>
                    <div className="text-ui-body font-black text-slate-700 dark:text-slate-200">
                      1000{' '}
                      <span className="text-ui-caption font-normal text-slate-400">
                        张
                      </span>
                    </div>
                  </div>
                  <div className="p-2 rounded-lg bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
                    ...
                    <div className="text-ui-body font-bold text-slate-700 dark:text-slate-200 truncate">
                ...
                <Button
                  className={cn(
                    'w-full h-control-compact text-ui-label font-bold shadow-md transition-all active:scale-95',
                    isTrading
            ...
            <Card className="flex-1 min-h-[300px] flex flex-col border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
              <div className="px-ui-section py-3 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/20 flex items-center justify-between">
                <span className="text-ui-label font-bold text-slate-600 dark:text-slate-300 flex items-center gap-2">
              ...
              <ScrollArea className="flex-1 bg-white dark:bg-slate-950/20">
                <div className="divide-y divide-slate-50 dark:divide-slate-900">
                  {paginatedOrders.map(order => (
                    ...
                    >
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-ui-caption font-bold text-slate-700 dark:text-slate-300">
                          {order.name}
                        </span>
                        <span className="text-ui-micro font-mono text-slate-400">
                          {order.time.split(' ')[1]}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-2">
                          <Badge
                            variant="secondary"
                            className="h-4 px-1 text-ui-micro bg-slate-100 dark:bg-slate-800 text-slate-500 font-mono"
                          >
                            {order.code}
                          </Badge>
                          <span className="text-ui-caption font-mono font-medium text-rose-500">
                            {order.rate}
                          </span>
                        </div>
                        <span className="text-ui-caption font-medium text-emerald-600">
                          {order.status}
                        </span>
                      </div>
                    </div>
                  ...
                  {!paginatedOrders.length && (
                    <div className="p-ui-section text-center text-ui-caption text-slate-300">
                      No orders found
                    </div>
                  )}
                </div>
