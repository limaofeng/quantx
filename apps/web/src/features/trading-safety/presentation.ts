...
const ACCOUNT_SAFETY_REASON_LABELS: Record<string, string> = {
  ...
  QMT_CONTROL_DEPENDENCY_UNAVAILABLE: 'QMT Agent 控制依赖暂不可用',
  QMT_CONTROL_TRANSPORT_LOST: 'QMT Agent 控制传输已中断',
  QMT_CONTROL_SESSION_REPLACED: 'QMT Agent 控制会话已被替换',
  QMT_DEVICE_REVOKED: 'QMT Agent 设备授权已撤销',
  QMT_API_RESTARTED: 'QuantX API 已重启，Agent 正在重连',
