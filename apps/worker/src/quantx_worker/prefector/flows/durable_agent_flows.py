  ...
  try:
    ...
    while asyncio.get_running_loop().time() < deadline:
      ...
      if isinstance(heartbeat_at, datetime):
        age = (
          datetime.now(timezone.utc) - heartbeat_at.replace(tzinfo=timezone.utc)
        ).total_seconds()
        progress = request.get("history_progress") or []
        if (
          -5 <= age < 30
          and request.get("history_session_active") == "true"
          and status in {"QUEUED", "DELIVERED", "RECEIVING"}
        ):
          current = next(
            (p for p in progress if p.get("request_id") == request_id), None
          )
          if current:
            phase = {
              "queued": "共享历史通道排队",
  ...
  async with AsyncSessionLocal() as db:
    queued = await TradeCommandService(db).enqueue_order_for_account(
      ...
      execution_ref=ExecutionOwnerRef.manual_command(
        "bond-repo:"
        + hashlib.sha256(
          (f"{str(account_id).strip()}:{str(idempotency_key).strip()}").encode("utf-8")
        ).hexdigest()
      ),
