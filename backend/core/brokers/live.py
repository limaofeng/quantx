...
class LiveBroker(BrokerBase):
  ...
  async def connect(self) -> bool:
    """连接到真实交易系统"""
    try:
      self.trading_service = TradingService(account_id=self.account_id)
  ...
  async def place_order(self, request: OrderRequest) -> OrderResponse:
    ...
    try:
      ...
      external_order_id = str(result.get("order_id") or "")
      if not external_order_id:
        raise RuntimeError("交易服务未返回委托编号")
  ...
  async def cancel_order(self, order_id: str) -> bool:
    ...
    try:
      ...
      result = await asyncio.to_thread(
        self.trading_service.trading_manager.cancel_order,
  ...
  def _convert_order_status(self, external_status: Any) -> OrderStatus:
    """转换外部订单状态"""
    from models.enums import OrderStatus as ExternalOrderStatus
  ...
  def _convert_external_order(
    ...
    """转换外部订单到内部格式"""
    from models.enums import OrderType as ExternalOrderType
