...
from database import WhereBuilder
...
class WhereBuilder(Generic[M]):
  ...
  使用示例:
  >>> from models import Instrument, Sector, SectorStock
  >>>
  >>> # 基础查询
  >>> where = (
  ...     WhereBuilder()
...
class BaseRepository(Generic[M]):
  ...
  def _apply_sort(self, query, sort: "Sort"):
    ...
    if sort and sort.sorted:
      # 动态导入避免循环导入
      from database.types import SortDirection
  ...
  async def find_page(
    ...
    # 动态导入避免循环导入
    from database.types import Pagination
