...
class OrderStatus(IntEnum):
  ...
  UNREPORTED = xtconstant.ORDER_UNREPORTED  # 未报
  """未报 - 订单尚未上报"""

  WAIT_REPORTING = xtconstant.ORDER_WAIT_REPORTING  # 待报
  """待报 - 订单等待上报"""

  REPORTED = xtconstant.ORDER_REPORTED  # 已报
  """已报 - 订单已上报"""

  REPORTED_CANCEL = xtconstant.ORDER_REPORTED_CANCEL  # 已报待撤
  """已报待撤 - 已上报的订单等待撤单"""

  PARTSUCC_CANCEL = xtconstant.ORDER_PARTSUCC_CANCEL  # 部成待撤
  """部成待撤 - 部分成交的订单等待撤单"""

  PART_CANCEL = xtconstant.ORDER_PART_CANCEL  # 部撤
  """部撤 - 部分撤单"""

  CANCELED = xtconstant.ORDER_CANCELED  # 已撤
  """已撤 - 订单已撤单"""

  PART_SUCC = xtconstant.ORDER_PART_SUCC  # 部成
  """部成 - 订单部分成交"""

  SUCCEEDED = xtconstant.ORDER_SUCCEEDED  # 已成
  """已成 - 订单已全部成交"""

  JUNK = xtconstant.ORDER_JUNK  # 废单
  """废单 - 无效订单"""

  UNKNOWN = xtconstant.ORDER_UNKNOWN  # 未知
  """未知 - 订单状态未知"""
...
class AccountType(str, Enum):
  ...
  def from_int(value: int) -> "AccountType":
    mapping = {
      xtconstant.SECURITY_ACCOUNT: AccountType.STOCK,
      xtconstant.HUGANGTONG_ACCOUNT: AccountType.HUGANGTONG,
      xtconstant.CREDIT_ACCOUNT: AccountType.CREDIT,
      xtconstant.FUTURE_ACCOUNT: AccountType.FUTURE,
      xtconstant.SHENGANGTONG_ACCOUNT: AccountType.SHENGANGTONG,
    }
...
class AccountStatus(int, Enum):
  ...
  INVALID = xtconstant.ACCOUNT_STATUS_INVALID  # 无效
  """无效"""
  OK = xtconstant.ACCOUNT_STATUS_OK  # 正常
  """正常"""
  WAITING_LOGIN = xtconstant.ACCOUNT_STATUS_WAITING_LOGIN  # 连接中
  """连接中"""
  LOGGING_IN = xtconstant.ACCOUNT_STATUSING  # 登陆中
  """登陆中"""
  LOGIN_FAILED = xtconstant.ACCOUNT_STATUS_FAIL  # 失败
  """失败"""
  INITIALIZING = xtconstant.ACCOUNT_STATUS_INITING  # 初始化中
  """初始化中"""
  CORRECTING = xtconstant.ACCOUNT_STATUS_CORRECTING  # 数据刷新校正中
  """数据刷新校正中"""
  CLOSED = xtconstant.ACCOUNT_STATUS_CLOSED  # 收盘后
  """收盘后"""
  ASSIST_FAIL = xtconstant.ACCOUNT_STATUS_ASSIS_FAIL  # 穿透副链接断开
  """穿透副链接断开"""
  DISABLE_BY_SYS = (
    xtconstant.ACCOUNT_STATUS_DISABLEBYSYS
  )  # 系统停用（总线使用-密码错误超限）
  """系统停用（总线使用-密码错误超限）"""
  DISABLE_BY_USER = xtconstant.ACCOUNT_STATUS_DISABLEBYUSER  # 用户停用（总线使用）
  """用户停用（总线使用）"""
