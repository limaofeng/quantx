...
from models.divid_factor import DividFactor
    ...
    for _, row in factors.iterrows():
      try:
        factor = DividFactor(
          ...
          interest=float(row["interest"]) if pd.notna(row["interest"]) else 0.0,
          stock_bonus=float(row["stock_bonus"]) if pd.notna(row["stock_bonus"]) else 0.0,
          stock_gift=float(row["stock_gift"]) if pd.notna(row["stock_gift"]) else 0.0,
          allot_num=float(row["allot_num"]) if pd.notna(row["allot_num"]) else 0.0,
          allot_price=float(row["allot_price"]) if pd.notna(row["allot_price"]) else 0.0,
          gugai=float(row["gugai"]) if pd.notna(row["gugai"]) else 0.0,
          dr=float(row["dr"]) if pd.notna(row["dr"]) else 1.0,
        )
