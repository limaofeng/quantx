...
class IntradayVolumeScanner:
  """Maintains latest xtquant whole-quote data and derives screening metrics."""
  ...
  async def start(self) -> bool:
    ...
    if sub_id is None:
      logger.warning("启动全市场量能扫描失败: xtquant 未返回订阅ID")
      return False
  ...
  def update_tick(self, code: str, tick: Dict[str, Any]) -> None:
    ...
    with self._lock:
      ...
      amount_delta = self._delta(
        current=amount,
        previous=state.previous_amount,
        fallback=volume_delta * price if price > 0 else 0.0,
      )
