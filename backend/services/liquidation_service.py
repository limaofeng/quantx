...
from core.trading.market_rules import AShareMarketRules
...
from repositories.conditional_liquidation_order_repository import (
...
class LiquidationService:
  ...
  def is_conditional_order_triggered(
    ...
    if avg_price and avg_price > 0:
      profit_pct = (float(latest_price) / avg_price - 1.0) * 100.0
      if target_profit_pct is not None and profit_pct >= target_profit_pct:
        return True, "target_profit_pct_reached", profit_pct
    elif target_profit_pct is not None and self._optional_float(order.target_price) is None:
      return False, "missing_avg_price", None

    target_price = self._optional_float(order.target_price)
    if target_price is not None and float(latest_price) >= target_price:
      return True, "target_price_reached", profit_pct
  ...
  def calculate_conditional_sell_volume(
    ...
    if sell_mode == ConditionalLiquidationSellMode.ALL_AVAILABLE:
      requested = available
      return self.market_rules.normalize_sell_volume(requested, available)

    if sell_mode == ConditionalLiquidationSellMode.PERCENT_AVAILABLE:
      ratio_pct = self._optional_float(order.sell_ratio_pct) or 0.0
      requested = int(available * max(0.0, min(ratio_pct, 100.0)) / 100.0)
    else:
      requested = int(order.sell_volume or 0)

    requested = min(max(0, requested), available)
    if requested < self.market_rules.lot_size:
      return 0
    return (requested // self.market_rules.lot_size) * self.market_rules.lot_size
...
class ConditionalLiquidationMonitor:
  ...
  async def _run(self) -> None:
    while not self._stopping.is_set():
      try:
        await self._evaluate_all_active_orders()
      except asyncio.CancelledError:
        raise
      except Exception as exc:
  ...
  async def _evaluate_all_active_orders(self) -> None:
    async for db in get_async_db():
      repo = ConditionalLiquidationOrderRepository(db)
      orders = await repo.find_active()
      break
