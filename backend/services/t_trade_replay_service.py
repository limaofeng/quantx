...
from services.t_trade_service import ACTIVE_RUN_STATUSES, TTradeService
...
class TTradeReplayService:
  ...
  def __init__(self) -> None:
    self.t_trade_service = TTradeService()
  ...
  def _replay_metrics(self, run: Any, backtest: Any) -> Dict[str, Any]:
    runtime = strategy_manager.get_run(run.id) if run else None
    if runtime and str(runtime.status.value) not in {"COMPLETED", "STOPPED"}:
      return build_t_trade_replay_metrics(runtime)
  ...
  def _project(self, run: Any, backtest: Any) -> Dict[str, Any]:
    ...
    current_time = getattr(getattr(runtime, "context", None), "current_time", None)
    ...
    error_message = (
      getattr(runtime, "error_message", None)
      if runtime
      else getattr(backtest, "error_message", None) or run.error_message
    )
    return {
      "run_id": run.id,
