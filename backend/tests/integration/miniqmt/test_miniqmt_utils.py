...
def test_batch_normalize_stock_codes():
    """测试批量股票代码标准化"""
    from miniqmt.utils.helpers import batch_normalize_stock_codes
...
def test_format_timestamp():
    """测试时间戳格式化"""
    from miniqmt.utils.helpers import format_timestamp
...
def test_calculate_trading_days():
    """测试交易日计算"""
    from miniqmt.utils.helpers import calculate_trading_days
...
def test_get_trading_calendar():
    """测试获取交易日历"""
    from miniqmt.utils.helpers import get_trading_calendar
...
def test_calculate_returns():
    """测试收益率计算"""
    from miniqmt.utils.helpers import calculate_returns
...
def test_calculate_cumulative_returns():
    """测试累计收益率计算"""
    from miniqmt.utils.helpers import calculate_cumulative_returns
...
def test_calculate_max_drawdown():
    """测试最大回撤计算"""
    from miniqmt.utils.helpers import calculate_max_drawdown, calculate_returns
...
def test_calculate_sharpe_ratio():
    """测试夏普比率计算"""
    from miniqmt.utils.helpers import calculate_sharpe_ratio
...
def test_calculate_volatility():
    """测试波动率计算"""
    from miniqmt.utils.helpers import calculate_volatility
...
def test_resample_data():
    """测试数据重采样"""
    from miniqmt.utils.helpers import resample_data
...
def test_validate_stock_code():
    """测试股票代码验证"""
    from miniqmt.utils.helpers import validate_stock_code

    # 测试有效代码
    assert validate_stock_code('000001.SZ') == True
    assert validate_stock_code('600036.SH') == True
    assert validate_stock_code('300750.SZ') == True

    # 测试无效代码
    assert validate_stock_code('invalid') == False
    assert validate_stock_code('') == False
    assert validate_stock_code('000001') == True  # 会被标准化为 000001.SZ

def test_batch_validate_stock_codes():
    """测试批量股票代码验证"""
    from miniqmt.utils.helpers import batch_validate_stock_codes
    ...
    assert results['000001.SZ'] == True   # 000001.SZ 有效
    assert results['invalid'] == False    # invalid 无效
    assert results['600036.SH'] == True   # 600036.SH 有效
    assert results[''] == False           # 空字符串无效

def test_format_money():
    """测试金额格式化"""
    from miniqmt.utils.helpers import format_money
...
def test_retry_on_exception():
    """测试异常重试装饰器"""
    from miniqmt.utils.helpers import retry_on_exception
...
class TestDataValidator:
    ...
    def test_validate_trade_data(self):
        ...
        assert validator.validate_ohlcv_data(cleaned_data) == True
