...
from core.trading import MarketDataSnapshot
...
class TestStrategyExecutor:
  ...
  async def test_start_run(self, strategy_executor):
    """测试 start 启动策略运行"""
    # Mock 数据适配器
    with patch('core.strategy_executor.adapter_manager.get_adapter_for_mode') as mock_get_adapter:
      ...
      # Mock BacktestBroker
      with patch('core.strategy_executor.BacktestBroker') as mock_broker_class:
  ...
  async def test_stop_run(self, strategy_executor):
    ...
    with patch('core.strategy_executor.adapter_manager.get_adapter_for_mode') as mock_get_adapter:
      ...
      with patch('core.strategy_executor.adapter_manager.release_adapter_for_mode') as mock_release:
        # Mock BacktestBroker
        with patch('core.strategy_executor.BacktestBroker') as mock_broker_class:
          mock_broker = AsyncMock()
          mock_broker.connect = AsyncMock()
  ...
  async def test_pause_and_resume_run(self, strategy_executor):
    ...
    with patch('core.strategy_executor.adapter_manager.get_adapter_for_mode') as mock_get_adapter:
      ...
      with patch('core.strategy_executor.SimulatorBroker') as mock_broker_class:
        mock_broker = AsyncMock()
        mock_broker.connect = AsyncMock()
        mock_broker_class.return_value = mock_broker
  ...
  async def test_different_modes(self, strategy_executor):
    ...
    for mode, broker_class in modes:
      with patch('core.strategy_executor.adapter_manager.get_adapter_for_mode') as mock_get_adapter:
        ...
        # Mock 对应的 broker
        broker_patch_path = f"core.strategy_executor.{broker_class}"
