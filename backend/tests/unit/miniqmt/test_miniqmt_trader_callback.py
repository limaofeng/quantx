...
from miniqmt.trading.trading_manager import (
...
class TestMiniQMTTraderCallback:
  ...
  def test_on_connected(self, callback):
    """测试连接成功回调"""
    with patch.object(callback.trading_manager, "handle_connection_event") as mock_handler:
  ...
  def test_on_account_status(self, callback):
    ...
    with patch.object(callback.trading_manager, "handle_account_status_event") as mock_handler:
  ...
  def test_on_stock_asset(self, callback, mock_asset):
    """测试资产变动回调"""
    with patch.object(callback.trading_manager, "handle_asset_update_event") as mock_handler:
      callback.on_stock_asset(mock_asset)
      # 验证任务提交

  def test_on_stock_position(self, callback, mock_position):
    """测试持仓变动回调"""
    with patch.object(callback.trading_manager, "handle_position_update_event") as mock_handler:
  ...
  def test_on_stock_order(self, callback, mock_order):
    """测试委托回报回调"""
    with patch.object(callback.trading_manager, "handle_order_event") as mock_handler:
      callback.on_stock_order(mock_order)
      # 验证日志包含订单信息

  def test_on_stock_trade(self, callback, mock_trade):
    """测试成交回报回调"""
    with patch.object(callback.trading_manager, "handle_trade_event") as mock_handler:
  ...
  def test_on_order_error(self, callback, mock_order_error):
    """测试委托失败回调"""
    with patch.object(callback.trading_manager, "handle_order_error_event") as mock_handler:
      callback.on_order_error(mock_order_error)
      # 验证错误日志

  def test_on_cancel_error(self, callback, mock_cancel_error):
    """测试撤单失败回调"""
    with patch.object(callback.trading_manager, "handle_cancel_error_event") as mock_handler:
  ...
  def test_on_order_stock_async_response(self, callback, mock_order_response):
    """测试异步下单回报回调"""
    with patch.object(callback.trading_manager, "handle_async_order_response") as mock_handler:
  ...
  async def test_concurrent_callbacks(self, callback, mock_order, mock_trade):
    ...
    with patch.object(callback.trading_manager, "handle_order_event") as mock_order_handler:
      with patch.object(callback.trading_manager, "handle_trade_event") as mock_trade_handler:
        # 同时触发多个回调
        for i in range(10):
          callback.on_stock_order(mock_order)
          callback.on_stock_trade(mock_trade)
