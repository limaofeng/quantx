...
from services.t_trade_global_monitor import TTradeGlobalMonitorService
...
async def test_reconcile_creates_one_run_for_dynamic_holdings_universe():
  ...
  service.position_service.refresh_from_miniqmt = AsyncMock(return_value={})
  service.position_service.get_positions = AsyncMock(
    return_value=[
      position("600000.SH"),
      position("000001.SZ"),
...
@pytest.mark.asyncio
async def test_failed_miniqmt_snapshot_does_not_reconcile_or_clear_universe():
  service = TTradeGlobalMonitorService()
  service._load_config = AsyncMock(return_value=config())
  service.position_service.refresh_from_miniqmt = AsyncMock(
