...
当前业务映射仍保留旧做 T/托管入口的 StrategyRun 运行容器；P1 只完成公共 owner 地基，
尚未把新做 T runtime 接入真实下单链：
...
| `T_TRADE_BATCH`、`LIMIT_UP_BOARD`、`FIRST_BOARD_PROMOTION_V2`、`ENTRY_PLAN` 退出 | `EXIT_PLAN/<plan_id>`；另存不可变 source execution owner       | 原 `StrategyRun` 的 `ExitPlanBook`，在 `step()` 前评估 |
| `MANUAL_POSITION`、`MANUAL_LIQUIDATION` 且无运行绑定                             | `EXIT_PLAN/<plan_id>`；来源由 `MANUAL_COMMAND` 见证            | 全局 `ExitPlanMonitor`                                 |
...
因此，以下文档仍描述待实施目标，不能据此假设当前已经存在独立
...
是 As-Is；P2/P3 的独立做 T runtime、公共退出 runtime 和跨域容量/准入仍只属于目标。
