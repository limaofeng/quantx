...
> 2026-09-07：P4 隔离 PAPER 执行与只读投影已部署至业务库 0056，Windows full/live 服务已重启。
> 双标的模拟闭环及故障路径通过隔离 PostgreSQL 验证；新 T owner 仍未注册 LIVE 订单 handler。
> 阶段证据和授权维护记录见实施方案，不代表 P5–P8 或整个交易系统的最终验收。
...
| 做 T 助手隔离 PAPER 执行                                                         | `T_ASSISTANT_EXECUTION/<execution_id>`（仅 `PAPER`）           | 独立 cycle、组合分配、公共 admission 与 PAPER ledger |
...
机会证据比较；无精确证据时持久化 `UNAVAILABLE`，不会退化为读取最新 UI 投影。
P4 将原候选证据与标准 `ALLOCATION_PENDING` TradeIntent 原子持久化，经 point-in-time
组合分配、公共排名 admission、最新 Tick Gate、Sizer/Risk/Capacity 进入隔离 PAPER 账本。
账本以独立源时间和受理时间记录模拟订单/成交，公共 ExitPlan 承接实际模拟成交后的退出。
原始 TTL 独立维护；分配/准入/订单仍整批提交，真实回报或严格终结审计才允许后续状态。
`paper_seed` 不可变且必须显式给出；缺少种子或必要证据时不伪造 READY。
只读 GraphQL/Web 投影按账户和 PAPER execution 隔离。公共 Agent runtime router 仍不注册
该 owner，legacy StrategyRun 继续承担现有 LIVE 入场，未提前执行 P7 清理或 LIVE 切换。

P6 隔离开发已提供新 T 源的 Engine 排空命令 `T_ASSISTANT_DRAIN_ENTRY`，仅阻断新 ENTRY、
审计终结未投递意图，保留原订单和退出义务。0060 迁移补齐独立 T pending/correlation 的
身份约束（有 intent、无虚构 strategy id）；迁移仅经隔离测试，尚不代表业务库已升级或
新 T LIVE 入场已接通。

独立 T 的 schema 2 确认信封与成交后精确退出授权已隔离接通，绑定原 execution、候选和
退出保护范围；终态源仍使用公共 LIVE 账户/持仓锁维护退出。预览/消费、LIVE 分配和新源
入场回报 handler 仍待完成，不能据此视为已切换实盘。
...
独立做 T 的 PAPER producer 已实现，LIVE 订单仍未开放。
