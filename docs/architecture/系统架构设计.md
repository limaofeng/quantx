...
| 当前协议 | Agent 控制协议 `1.1`；行情子协议 `quantx.market.v2` |
...
7. OrderSizer、A 股规则、后置订单风控和 `AccountCapacityService` 计算最终合法订单；
8. pending order 与 `trade_command_outbox` 在同一持久事务中建立；
9. API Agent Hub 领取命令，发送前重新校验账户、Agent、行情和命令状态；
10. QMT Agent 再做模式、白名单、过期、对账和本地安全检查后调用 XTTrading。
...
TRADE 是独立报告；终态声明的累计成交量领先于真实 TRADE 时必须保留关联并进入
`RECONCILE_REQUIRED`，不能凭委托终态伪造成交或释放库存。

Engine 把原始报告转换成稳定业务键的 `strategy_runtime_events`，按运行串行应用，并
...
当前代码仍处于“通用 StrategyRun + 部分独立 ExitPlan”的架构阶段：

| 业务                                                                             | 当前持久 owner / 运行容器                     | 当前调度位置                                           |
| -------------------------------------------------------------------------------- | --------------------------------------------- | ------------------------------------------------------ |
| 普通策略                                                                         | `StrategyRun`                                 | `StrategyManager`                                      |
| 做 T 助手入场                                                                    | `StrategyRun`                                 | 原运行的串行策略队列                                   |
| 打板助手入场                                                                     | `StrategyRun`                                 | 原运行的串行策略队列                                   |
| 托管买入计划                                                                     | `StrategyRun` 绑定当前计划版本                | 原运行的 managed entry strategy                        |
| `T_TRADE_BATCH`、`LIMIT_UP_BOARD`、`FIRST_BOARD_PROMOTION_V2`、`ENTRY_PLAN` 退出 | SELL 意图由 `ExitPlan` 拥有；源运行仍负责评估 | 原 `StrategyRun` 的 `ExitPlanBook`，在 `step()` 前评估 |
| `MANUAL_POSITION`、`MANUAL_LIQUIDATION` 且无运行绑定                             | `ExitPlan`                                    | 全局 `ExitPlanMonitor`                                 |

因此，以下文档描述的是待实施目标，不能据此假设当前已经存在独立
`TAssistantExecution`、`BoardAssistantExecution`、`EntryPlanRuntime` 或统一的独立
`ExitPlanRuntime`：
...
| 行情接入    | 独立 Market Gateway；控制 `1.1` 与行情 `quantx.market.v2` 分离 |
