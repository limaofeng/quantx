...
回归测试包括容量超时、真实 SQLAlchemy 会话关闭期间的重复取消、真实 SDK 搭配
本地假模型并行调用真实注册工具的取消、超时、失败结算与审批暂停，以及隔离
测试库上的 PostgreSQL `2+1` 连接池复现。工具回归还覆盖开始/结束审计、事件写入
期间的会话关闭和已排队工具在运行退出后的拒绝执行。
