...
命令入箱服务也必须经过同一工作额度。共享 `EngineCommandService` 的 `enqueue/get`
只有在内部会话关闭并归还物理连接后才能返回或传播取消，确保外层工作额度覆盖
真实连接的完整生命周期。
...
取消时，数据库会话的关闭由独立清理任务完成；即使等待关闭期间再次收到取消，
也必须等连接归还后才释放并发额度，并继续向调用方传播取消。
取消、超时或事件持久化失败时，Runtime 显式停止 SDK 流并消费至清理结束，
不能只取消外层事件消费者。SDK 已开始取消时不重复打断其清理；正常完成和
审批暂停不额外取消。

SDK 流结束不代表它分派的函数工具已退出。每次运行通过统一 `_invoke_audited`
入口登记实际执行工具的任务，登记范围包含开始审计、业务回调、结束审计及其
数据库会话关闭。退出流式循环时先拒绝尚未开始的工具调用，再等待 SDK 流结束，
取消并等待仍属于本次运行的工具；已在取消清理中的工具不再次打断。
这条清理路径在流消费失败时也必须完成，外层重复取消不能让它脱离运行所有权。
已登记工具任务及其 Runtime `database_session` 清理完成后，才结算失败、完成消息
或进入审批暂停，也才归还运行并发名额；不使用全局任务扫描、不依赖 SDK 私有
任务字段、不扩大数据库连接池。
...
回归测试包括容量超时、真实 SQLAlchemy 会话关闭期间的重复取消、共享命令入箱
服务取消时的连接与工作额度共同所有权、真实 SDK 搭配本地假模型并行调用真实注册
工具的取消、超时、失败结算与审批暂停，以及隔离测试库上的 PostgreSQL `2+1`
连接池复现。工具回归还覆盖开始/结束审计、事件写入期间的会话关闭和已排队工具在
运行退出后的拒绝执行。
数据库测试只执行 `SELECT 1`，强制只读事务，验证旧连接关闭期间心跳仍可用且新
业务不能抢占额度；不写业务表、不调用外部模型、不接触交易链路：
