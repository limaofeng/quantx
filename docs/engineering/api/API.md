...
版本，不读取无版本本地表。生产端从 COMPLETED 原生请求的覆盖和内容证明解析固定
storage_version；15:01 后发起且 download=true 的整日源优先，否则归档覆盖其声明
的分钟，其余分钟读取同一个原生固定版本。新修订未验证时同时隐藏该分钟的旧归档
和原生版本值。默认 tick、1d 分区读取也要求原生固定版本；缺少版本返回 503，
不从 canonical 表推断发布。版本按源请求创建时间排序，较旧请求迟到不覆盖新源；
同一创建时间存在不同版本则拒绝选择。
...
这些页不证明全天完整。默认 latest-daily 已接固定版本目录：原生模式在一次有界
目录查询中按标的/日期选择最新已完成源，校验内容证明并检查该时间的全部候选
是否有版本冲突；单分区与日线目录均不以截断后的前两项代替冲突检查。
每分区只返回一个代表及冲突标志，日线最多 32×62 项、4 MiB；
开发模式继续选择已发布交付版本。Flight 查询在 LIMIT 前同时限定标的、日期和
版本，防止一个多分区旧批次因其他分区仍被选中而混入已更新日期。无发布版本时
不查 canonical 表，返回空快照；快照仍不构成覆盖证明。其他直接入口仍待迁移。
Engine 默认生命周期已接范围登记和有界归档发送，已移除实时聚合的直接 Influx writer。
归档 Tick 使用独立 ARCHIVE FIFO（每标的 8 批）。首快照、队列溢出、回调失败及
全局重新同步会标记本机投递中断，不改源代次、不关闭交易消费者；下个快照重置
聚合起点。只有从前一分钟连续观察至下一分钟的分钟才 sealed=true，首分钟、
中断分钟和跨空窗分钟保持未封口。封口表示本机源观察窗口完成，不替代全天原生
覆盖证明；缺口仍由原生补数证明关闭。Tick 订阅建立前先将范围意图提交到本机 SQLite，再异步登记；
