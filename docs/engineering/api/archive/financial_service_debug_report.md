# save_batch_financial_data 历史调试报告

> 历史迁移资料：本文记录的是旧 `backend/` 单体目录时期的排障过程，其中
> 路径与命令不再是当前运行入口。当前实现位于
> `packages/infrastructure/src/quantx_infrastructure/services/`，运行和测试
> 请以仓库根 `README.md` 与 `docs/engineering/api/README.md` 为准。
