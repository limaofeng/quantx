...
Influx。范围后台登记最多 4 次、每次 3 秒，退避 1/2/4 秒；未登记成功不发送归档，
仍继续实时推送。每代次最多 10000 个内存范围，发送仍受 512 项/2 MiB 限制。
当前聚合来源为 latest-only，统一 sealed=false。登记前崩溃恢复、完整分钟来源及
其他消费者尚未完成，不能部署为完整切换或将本批当作范围恢复验收通过。
