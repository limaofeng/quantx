...
上线本批前应用迁移至 `20260910_0089`，并停止、移除旧 Prefect deployment
...
过期后的新交付从原请求固定版本重建；原生收据的逐日覆盖同时保存 content_sha256，
重建核验分区行数和内容摘要，不读取后来写入的 canonical 行。旧收据缺少固定版本
或分区摘要时明确要求迁移，返回 INCOMPLETE，不仅凭正行数重建。分页读取每页最多
2000 行/4 MiB，总计 500000 行/512 MiB/60 秒；取消后等待实际读取线程退出。
行情交付当前使用 v2 清单，版本摘要绑定原源 ID、创建时间、原始范围、原生/分区摘要和
参考数据；生产端与开发端必须同时升级。旧 v1 交付不默认兼容，恢复入口以
SOURCE_PROVENANCE_MIGRATION_REQUIRED 阻断并保留原清单、文件及预算，读取目录排除 v1；
上线前须显式迁移这些证据，不能删除后新建交付来刷新预算。目前尚未执行现场迁移。
开发端检查 SHA256、协议、范围与行数，写入前对比源分区内容摘要，幂等导入后再次回读；
已下载分片可复用。REMOTE 归档恢复只检查原需求绑定的交付，源在目标交易日上海 15:01
后创建且 download=true，并且本地发布证明和源摘要一致，才将缺口关闭。盘中/缓存来源
及未发布交付保持等待；证明损坏记录原因，不改变需求关联。此证明不授予策略准入或交易权限。
...
请求与覆盖/内容证明，盘中采集或仅缓存读取不能遮蔽后来归档。原生默认摄取已写入
ticks_versions、kline_1m_versions、kline_1d_versions；完成收据发布内容寻址版本，
默认 Data API 分区读取只选择有证明的版本。旧 canonical 收据不能作为新版本证明；
旧摄取检查点返回 NATIVE_STORAGE_VERSION_MIGRATION_REQUIRED，保留原身份和预算。
默认 latest-daily 也已切换到原生固定版本目录，跨分区版本在查询 LIMIT 前精确过滤。
导出重建也已切换到原请求固定版本及分区摘要。上线前仍须完成旧证据迁移和其他消费者切换，不能直接
部署本批造成已有数据不可读。迁移 0087 增加持久化
恢复范围，API 接收修订前强制检查范围，就绪检查包含范围表；若已有旧归档证据，
迁移明确拒绝并要求先完成原证据的范围映射，不自行猜测崩溃前的起点。
Engine 默认启动/停止已管理归档客户端、范围登记和有界发送器，实时聚合不再直接写
Influx。Tick 订阅建立前先提交本机运行目录下 engine-archive-intents.sqlite，后台
登记每次 3 秒、累计最多 4 次，尝试前落盘预留并持久化 30 秒间隔；重启不返还预算。
Data API 不可用时仍可建立实时订阅，未登记成功不发送归档。SQLite 不可写或容量
耗尽时拒绝新增无恢复依据的订阅，不绕过落盘。每代次最多 10000 个内存范围，跨
代次待确认意图最多 10000 条，SQLite 主文件最多 16 MiB（另有事务日志），发送仍受
512 项/2 MiB 限制。不要删除该文件解除阻塞；确认 PG 已持久化原范围后才移除本机
待办。耗尽的未确认意图保留，须显式恢复处理，不通过重启重置。
新 Engine 通过 scopes/recover 补交旧代次未确认范围，服务端按恢复观察时间冻结
结束边界；旧源仍不能新增修订。默认归档订阅使用每标的 8 批的独立 FIFO，溢出
丢弃本地积压并显式标记观察中断，不触发交易消费者 SYNCING。首快照、中断分钟
和跨空窗聚合未封口；连续跨越前后边界的分钟才封口。封口只证明本机观察窗口，
不代表全天原生采集完整，恢复范围仍依赖整日证明。耗尽意图的恢复操作、其他
消费者和完整故障验收尚未完成，不能部署
为完整切换；本机磁盘与 PG 同时不可用时不会声称新订阅具有恢复依据。
迁移 0088 增加范围结束时间、逐日游标和原补数需求关联。默认 Data Worker 独立循环
每轮至多推进一个范围的一天，需求创建与游标在同一租约事务提交；日历不足等待 5
分钟，需求容量不足等待 30 秒，盘中等到 15:01。源结束仅冻结归档边界，不代表数据
缺口关闭；WAITING 仍须匹配补数证明。已接受修订可在范围结束后重放，但拒绝新增。
迁移 0089 增加恢复证明、完成时间和到期探针。AGENT 范围可由已发布的整日原生固定
版本关闭，证明同时保留原需求、原关联源和实际证明源；无充分证明则保留 WAITING
及具体原因，每 5 分钟复核。REMOTE 的整日来源证明接线仍待完成，不按交付完成状态
自动关闭。存在任何恢复记录时禁止降级删除证明或探针记录；本批迁移尚未应用生产。
