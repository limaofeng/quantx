...
状态条的历史色块按区间内最差状态着色，悬停显示完整区间；服务恢复不会抹去该
区间内的故障。最右侧单列“最新”色块，使用 summary 的状态和采样时间，随摘要
刷新更新，不受历史分桶或历史请求失败影响。该色块表示最近一次采样，Monitor
不可访问或数据陈旧时仍须结合页面告警判断，不能视作实时健康保证。
