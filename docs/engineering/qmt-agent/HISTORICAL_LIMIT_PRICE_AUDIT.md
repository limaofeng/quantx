...
- P5 取数按标的、交易日关联日 K 的两个参考字段，绝不回退到历史 Tick 上的旧值。
  ...
  历史日 K 两字段为空是正常状态。按用户后续确认，P5 采用 CHECK_WHEN_AVAILABLE.v1：
  仅日 K 有可用值时检查涨跌停，缺值不阻断回测；不要求补造历史值或改换样本日期。
  数据分区单独记录 daily_price_limits，涨跌停字段不计入 missing_reference_fields，
  不触发 REFERENCE_REQUIRED；最小价位等必需资料缺失仍阻断。实时风控不受此调整影响。
