...
QMT Agent 是 Windows 生产主机上的独立进程，由统一 `ops/quantx.ps1` 启停。
...
当前个人生产环境固定登记 `http://192.168.5.6:8080`，并只在受控私有局域网
...
`paper` 或 `live`。日常 `live` 使用 `ENV=production`（单独授权的真实交易测试可使用 `ENV=testing`），且同时
...
历史下载使用项目适配器 `xtdata_history_download.py` 等待原生完成事件，避免已审核 SDK
包装器的十秒轮询。适配器按 `xtdata.py` 指纹限定支持范围，SDK 升级前必须重新验证；
未知指纹只阻止历史下载。下载软截止 25 秒，父进程保留 30 秒监督；原生异常会先回收历史
子进程再重建，普通数据校验异常仍按请求隔离。`history_timing` 日志记录工作单元、
回调至返回、读取、编码、压缩及上传耗时。诊断证据、指标口径与生产验收状态见
[历史补数诊断及优化交接](../deployment/HISTORY_BACKFILL_PERFORMANCE_DIAGNOSIS_20260908.md)。

完成回调可能早于 XTData 缓存可读；显式下载后对缺失、空帧及全部为无效占位行的标的
按 0.1/0.3/0.6/1 秒做最多四次缓存重读，不重下数据、不覆盖已有有效行的标的。
占位判定与上传过滤共用谓词；持续缺数保留无数据摘要，不推断停牌。`cache_no_usable_rows`
记录原始/有效/过滤行数，`frame_projected` 记录逐标的行数及时间范围，结合重试和下载
时序区分空帧与占位帧，不推断 XTData 内部缓存更新实现。API 每台设备最多保留两个未完成历史请求：
`DELIVERED/RECEIVING` 独占原生派发，上一请求冻结 manifest 后，下一次下载可以与其
入库回读重叠；两个请求都待入库时停发。源请求最终完成条件、字节/磁盘配额、开发窗口、
生产优先和交易健康约束保持不变。
