...
`compute_daily_signals=true` 的日级指标快照。

日级指标快照默认覆盖全部沪深股票和 ETF。持久化完成请求必须为每个目标代码
...
任何复权因子写入都会在同一个数据库事务内撤销现有日级指标快照的发布资格：
`daily-indicator-v1` 快照行不再携带当前计算版本，对应的 `success` 或
`scoped_success` 运行记录转为 `invalidated`。这样，复权因子锁释放后的历史修订不会
让 API 继续把旧指标计算结果认证为可用；必须重新计算并完整发布快照后才能恢复
选股结果。
