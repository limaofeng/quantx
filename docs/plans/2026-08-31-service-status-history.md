...
| `GET /monitor/api/v1/incidents` | 总览最近事故、独立历史列表 | `range/targetId/page/pageSize`，成对 `asOf/maxIncidentId`；返回总数和当前页；取消过期请求并忽略迟到响应 |

事故按开始时间和 ID 双重倒序，范围匹配包含跨边界的长事故，不再截断为 200 条。
第一次响应的 `asOf/maxIncidentId` 固定本轮时间范围与入库 ID 上限，后续翻页成对
回传；截止前开始但延迟入库的探测也不会把新事故挤入当前页。
恢复时间和最近原因不是不可变快照，保留期清理后总数可以减少，越界页会回到末页。
刷新、切换指标或范围后建立新截止点。页面刷新重新建立截止点，不将其存入 URL。
局部重试只重发失败资源，保留页码和分页边界；顶部刷新回到第一页。
切换指标或范围同时更新当前概览，事故持续时间单独以事故响应的 `asOf` 计算。
...
本功能没有未决产品或 API 选择。以以下“2026-08-31 复盘修复”中的最终复验为准：
Caddy codegen、完整类型/UI 检查、lint、Monitor/前端测试及文档构建均已通过；
Vite 产物已生成，CSS 总包预算在并行任务调整后复验通过。此前的契约缺失和预算
超限不再是待解决阻塞；上方初次实现验证中的失败仅保留为历史记录。
交易执行、主运行时重启及并行任务的实现改动仍不在本次范围。

## 2026-08-31 复盘修复

- 分页增加入库 ID 上限，覆盖“探测开始早于截止点、事故入库晚于第一页”的场景；
  空库水位 0 也保持稳定。API、客户端及文档同轮切换，不保留仅传 `asOf` 的旧契约。
- 当前概览随指标/范围刷新；活动事故时长使用事故查询自己的 `asOf`。
- 概览、曲线和事故各自重试，保留当前页及分页边界；顶部刷新才重新查询第一页。
  切换经过失败请求再返回原指标时，也会建立新边界。
- 回归：针对性 API/storage 9 项通过；最终工作区 Monitor 54 项、前端 707 项全部通过。
  Caddy codegen、完整 `npm run check`、`npm run lint`、本次文件 ESLint/Prettier、
  Python Ruff 与差异检查通过。并行行情语义调整期间的旧断言失败已在最终重跑消除。
- 构建：Vite 产物生成；CSS 230.7 KiB 仍超过既有 230 KiB 总包预算，未放宽预算。
  本次没有新增样式。文档构建随完整 check 通过。
  随后并行任务完成包体调整，当前产物的 `npm run budget:check --workspace apps/web`
  复验通过；包体配置差异不属于本次提交。
- 浏览器：真实 Web / Caddy 一年历史共 15 条，切换每页 10 条后分页为 10/5；
  URL、指标/范围切换、顶部刷新返回第一页正常，检查时控制台无 error/warn。
  故障重试由隔离前端测试覆盖，不对运行中的服务注入故障。
- 仅通过统一运维入口重启独立 Monitor 加载新接口；没有操作 API、Engine、QMT Agent。
  与并行任务共享文件时只提交分页相关差异，不吸收行情健康或卖出管理改动。
