...
正式验收状态仍是 `NOT_RUN`：尚未重新执行上述 canonical 准备命令，没有
canonical cutover token，没有执行正式 20 日 BACKTEST，也没有生成 rollout PASS。
下一步直接重跑准备命令；准备成功后使用报告生成的 token 执行正式回放。5 日
替代 PAPER 仍按独立门禁执行，不得由 20 日结果或合成压力代替。
...
3. **尚未完成的长时/外部验证**：固定 9,600 Tick 压力已完成，不再作为本轮待办；既有 durable ingestion 阻塞已经解除，但 20 日真实行情准备尚未重跑，正式回放和 5 日替代 PAPER 均未完成，不能因本次压力通过、摄取恢复或准备代码完成而省略。
...
ingestion 阻塞已经解除；但 canonical 准备尚未重跑，故 canonical token、正式
