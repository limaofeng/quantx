...
本计划曾在早期草案中建议以 Quant Red 作为默认 Studio 主色。该建议已于
2026-08-24 废止：红色与 A 股上涨、买入方向和风险提示的语义距离过近，容易把
“当前选中”误读为交易事实。

现在的唯一权威规则是
[`Web UI/UX 设计系统`](../engineering/web/UI_UX_DESIGN_SYSTEM.md)：

- 默认交互主色：Quant Blue，`#2563EB`；hover 使用 `#3B82F6`，选中边框与
  focus 使用 `#60A5FA`。
- 导航、选中态、焦点态、普通 CTA、刷新与重试必须使用蓝色交互语义。
- A 股上涨/正收益保持红色；下跌/负收益保持绿色；持仓下跌可使用浅蓝；风险、
  警告、成功均使用各自状态色。
- 主色 token 必须集中在 `apps/web/src/index.css` 与 Studio theme helper，不能在
  业务组件散落色值。
...
- 默认主色：Quant Blue；颜色与交互规则以
  [`Web UI/UX 设计系统`](../engineering/web/UI_UX_DESIGN_SYSTEM.md) 为准。
...
后续仅在需要独立演示页或 Storybook 类验证环境时另行决策；不得重新引入红色
作为默认交互主色，除非先更新权威设计系统并完成全量语义审查。
