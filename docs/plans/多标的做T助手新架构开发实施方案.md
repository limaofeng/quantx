...
- 退出授权组合缺口已修复：公共模板追加的来源投影仅在与签名来源完全一致、且原签名
  模板未包含该字段时从比较视图移除；冲突与原签名字段继续拒绝，挑战和持久化模板不
  改写。生命周期仍开放但已有成交时保留 PARTIAL_FILLED，避免回退 EXECUTION_PENDING
  导致已成交退出授权一直等待；最终 FILLED 仍由生命周期收敛决定。
  隔离组合加入真实用户/设备权限检查与显式合成持仓（新增 100 当日不可卖、旧仓 1000
...
- LIVE 续单当前风险复核组件已完成：复用实际 portfolio/持仓归因/OrderSizer/RiskChecker，
  保留原已成交敞口、现金与旧仓退出义务，仅豁免本批次已有入场的同标的重复机会阻断。
  其他同标的义务、未进入完整快照的原单余量、不可替换旧仓、组合/行业/单标的额度仍阻断；
  冻结五档报价副本，并要求估值 mark 与订单报价的价格、时间一致。剩余分配金额扣除累计
  成交，风控后再次核验确认，返回请求有效期取原生命周期与分配有效期较早者。
...
- AUTO successor 内部消息箱接线已完成：`T_ASSISTANT_PREPARE_LIVE_AUTO_SUCCESSOR` 严格接收
  ...
  P7 reducer 离线测量入口 `ops/benchmark_t_assistant_reducer.py` 已补齐：Conda quantx Python
  直接运行，默认 3 个合成标的各 1024 ticks，比较批量 1/8 的最终机会状态哈希并校验每条
  accepted tick 恰好评估一次；记录 p50/p95/max 和总耗时。固定 512 ticks、2 秒与 ring
  溢出门保持不变，注入超界后必须无候选并有明确 rewarm 原因。CLI 总输入上限 32768 ticks。
  本机 macOS/arm64、Python 3.13.11 基线：逐 Tick 39.42 秒、批量 8 为 7.72 秒，分别
  3072/384 次 reduction，最终状态一致；三类 fail-closed 检查通过。补充 1×64 smoke
  校验完整计数及重暖原因，Ruff/差异检查通过。证据 `p7-reducer-benchmark.json`、
...
- 剩余开发顺序：
  ...
  **6 项 API、27 项原生模拟器测试通过**，含引用错绑、设备变化、旧时刻与锁定后恢复；
  ...
  不纳入服务端提交或阶段完成证据。排空恢复补齐配置级持久化阻断：空会话、头绑定已解除
  或标记损坏时，monitor 不采用、停止或重建旧执行，仅失效其入场权限；公共 BUY 分发按
  账户配置头加锁，实际策略类仍为旧做 T 的新 run ID 也不能绕过原排空标记，同账户其他
