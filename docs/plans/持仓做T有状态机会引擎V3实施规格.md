...
```text
总览 | 信号 | 诊断 | 仓位与批次 | 运行动态 | 参数
```

- **总览**：全局运行状态、数据健康分布、待确认信号、活动批次、关键风险。
- **信号**：只展示持久化机会评估，不混入普通持仓行；采用全宽业务动态行，点击后在原行下展开候选、评分、阻断、追溯与确认详情。
- **诊断**：漏斗、blocker、分数分布、FSM 停留和历史评估。
- **仓位与批次**：实盘与回测复用同一工作区；实盘提供“当前仓位 / 历史批次”，回测提供“期末仓位 / 历史批次”。批次只使用执行事实，不混入候选状态。
- **运行动态**：TradeIntent、委托、成交与拒绝审计；从批次抽屉跳转时保留 `batchId` 筛选。
- **参数**：有类型的 signal policy 编辑、预览、版本冲突处理和重热提示。

#### 15.2.1 仓位与批次工作区

“仓位与批次”是实盘和回测共用的唯一批次浏览入口，回测总览不再保留重复的完整批次表。工作区遵守以下契约：

- **当前/期末仓位**按标的聚合剩余活跃数量，支持单表搜索与状态筛选；没有权威定向行情时，实盘最新价和浮动盈亏显示为缺失，不以缓存价或零值伪装。
- **历史批次**只展示终态批次，实盘默认最近 30 日，提供来源、结果和关键字筛选；回测限定当前回放任务，不能与实盘批次混合。
- 历史区顶部展示终态批次数、已完成批次、净增量和资金利用率摘要；持有时间和资金利用率按投入本金加权，不能用批次数做简单平均。
- 表格保留批次身份、来源、状态、买入、卖出、税费、等待/资金利用率、净增量和退出原因；点击行在右侧打开详情抽屉，不依赖移动端布局。
- 实盘手续费、税费、净增量与资金利用率统一标注“规则估算”；回测使用任务冻结的确定性费用模型。缺失成交时间、成交金额或退出事实时显示“不完整”，不得用当前时间或推导值补造事实。
- 抽屉串联买入、卖出、费用、指标来源和退出原因，并提供“查看运行动态”入口；跳转后将批次编号写入运行动态筛选，返回工作区时保留原视图上下文。
- 历史查询采用游标分页，前端可持续加载完整 30 日范围；当前仓位按剩余数量优先排序，历史批次按终态时间倒序。
