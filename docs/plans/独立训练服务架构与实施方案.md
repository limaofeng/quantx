...
- 跨环境发布新增显式控制面 CLI：开发侧核对成功 FINAL_EVALUATION/DEVELOPMENT 数据库血缘，封装代码、依赖、种子、模型和人工审核证据；目标端按固定 bundle_id 校验、实际 CPU 加载推理后原子安装，独立登记为 CANDIDATE。数据库失败可重试且不覆盖已有审批阶段；macOS 拒绝远程数据库及生产导入。52 项受影响测试通过，证据为 `.runtime/trainer-deployment/selection-release-acceptance.{log,xml}`，包含真实小型 CPU 模型、离线 CLI、无训练表的 SQLite 目标登记、篡改拒绝、来源校验及已有 API 回归。修复了既有制品加载器循环变量覆盖 manifest 文件哈希的问题。测试中的评估数据为合成夹具；真实训练产物、Windows 目标导入与显式绑定尚未验收，阶段 5 不据此关闭。
- 发布包原生验证：`c67d5652` 独立只读校验代码已在 Windows `quantx_trainer` 账户下安装并复核合成评估夹具包，实际单线程 CPU 推理成功。macOS LightGBM 4.7.0 与 Windows 4.6.0 的校准预测哈希相同，证据为 `.runtime/trainer-deployment/cross-platform-release-c67d5652.json`、`windows-runtime/native-release-c67d5652.json`。新代码目录最初缺少运行账户只读权限，补齐后通过；正在运行的 aa810472 Trainer、Conda 安装和生产状态均未变更。另在本机独立 `quantx_release_c67d5652_test` PostgreSQL 库实际执行 CLI 导入，仅有模型登记表；CANDIDATE 不进入推理范围，显式 SHADOW 后可按固定 manifest 哈希加载。证据为 `release-postgres-import-c67d5652.json`、`release-postgres-shadow-c67d5652.json`、`release-postgres-repeat-c67d5652.json`。该验证不是 Windows 生产控制面的登记、做 T 模型绑定或真实 GPU 训练验收。

- 当前状态：实现中。最新独立 Trainer 全量回归 370 通过、5 项 Windows 专项跳过，见 `.runtime/trainer-deployment/trainer-runtime-regression.{log,xml}`；其后状态文件竞争的受影响测试 25 通过、1 项原生跳过，见 `status-sharing-tests.xml`。两项新增 Windows 专项（隔离环境系统目录、持读句柄期间发布心跳）已在真实低权限账户下分别通过，证据为 `windows-runtime/native-known-folder-8714100c/`、`native-status-sharing-aa810472/`。最新组合回归覆盖 Trainer 全部测试、主机门禁、训练仓储/任务查询，以及 Research CLI/准备/GPU/工作流/CPU 线程和 Worker 准备流程：501 项通过，4 项 Windows 原生 ACL/Job 测试跳过。完整日志、JUnit 与摘要位于 `.runtime/trainer-deployment/regression-current.log`、`regression-current.xml`、`regression-current.json`；该结果不证明 Windows/GPU 部署或跨机器验收完成。
- 生命周期与状态：独立配置/预检、serve/up/down、drain/resume、根运维入口及按运行/准备归属读取的脱敏日志已接入；Windows Job 包含、组退出证明和停止后数据库恢复已有实现。status 离线显示服务身份、启动代码证据、心跳、准入、全机资源及配置绑定的 CPU/GPU/任务快照。任务快照用单条只读查询，运行中优先，最多 50 条且显式截断；仅反映数据库状态，不证明进程存活或已排空。90 秒后隐藏旧资格与任务。最新查询/缓存/调度关联回归 33 项通过，含实际 SQLite 查询到文件快照与旧 Worker 测试；证据为 `.runtime/trainer-deployment/activity-status-tests.xml`。两类调度器现记录实际返回的稳定原因及开始/结束决定，状态按时间、配置和调度类型校验；46 项相关测试通过，证据为 `.runtime/trainer-deployment/dispatch-status-tests.xml`。训练领取已原子改为返回“观察到运行中训练 / 无可领取行 / 提交完整性冲突”三种结果，51 项相关回归通过，含真实 SQLite 回滚和调度展示，见 `.runtime/trainer-deployment/claim-reasons-tests.xml`；Windows 原生停止、残留进程、断联和数据库整链仍待验收。
...
- 部署输入：Windows 当前固定提交 `aa8104726b98f7a282348df8f3b7b4e9cb48cc24`，3,010 个文件，目录 `C:\Users\limao\QuantXTraining\code\source-aa810472`；源码包位于 `.runtime/trainer-deployment/code-aa810472`，清单 SHA256 为 `39122cbd75a1fc205d2e047e6136a89d1fbf895702e93b97df8bc06faee5c828`。外部依赖按锁文件从 161 增至 163 条，仅新增已锁定的 importlib-metadata 9.0.0 / zipp 4.1.0，原 161 条未变；导出清单位于 `dependencies-21e2e29f`，后续提交锁文件未变。六个工作区包完成可编辑安装，pip check 通过，安装前后源码证据相同且 dirty=false。各旧版本包、安装结果和启动失败证据保留；只有确认服务/进程退出后才切换代码及依赖。
- 运行冻结：Windows 当前进程 ACL/危险权限预检已覆盖代码、Conda 全树及父目录替换；81 项相关测试通过；Windows 原生当前身份/可写临时文件拒绝冻结检查已通过，低权限账户的全树实际验收见下文。部署清单保留源码提交证据；训练结果与开发锁哈希覆盖 `source-evidence.json`，源码变更或父开发版本不一致拒绝执行。标准可编辑元数据识别及工作流回归 50 项通过。证据分别为 `.runtime/trainer-deployment/runtime-permissions-tests.xml`、`training-source-tests.xml`、`editable-source-tests.xml`；仍需完成有活动任务时的升级排空及运行期间冻结验收。
- 运行端：SSH 已通过显式本地密钥稳定连接。实际启动依次定位并修复：干净环境缺少 ProcessWorker 所需 importlib_metadata；公共数据库包导入时创建通用服务引擎；隔离环境丢失 SystemDrive 导致 Windows 公共目录解析失败。当前补齐依赖、延迟数据库连接导出、预检真实 Worker/流程导入、保留最小系统变量和固定错误类型/位置日志。f15b86f0 已实际进入 WORKER_LOOP，三个独立部署 READY，准备和训练调度完成；原生根入口 down 返回 GROUP_EXITED、database_state=RECONCILED，所有 pending 列表为空。8714100c 重启暴露状态文件读写竞争；Windows 原生持读句柄复现 WinError 5，已补最多 1 秒有界原子替换重试，持续错误仍拒绝发布。aa810472 已稳定进入 WORKER_LOOP；三个真实流程全部 COMPLETED，训练和准备快照均 FRESH、原因 TRADING_OR_POST_CLOSE_CRITICAL_WINDOW，保护时段不触发 GPU 探针。证据为 `trainer-first-protected-cycle.json` 与 `windows-runtime/runtime-checkpoint-aa810472.json`。训练和发布整链仍未完成。
- Windows 原生测试：在已安装的独立环境执行此前跳过的 4 项检查（当前身份不能误认自有临时文件为冻结、正常/强制退出回收后代、命名 Job 身份及整组回收），不加载根 conftest、不连接数据库、不运行训练。首次 3 通过、命名 Job 在 10 秒就绪期限内未生成文件；独立测量状态报告器约 1.7 秒初始化，未改代码重跑后 4 项通过（3.86 秒）。两次日志/JUnit/结果保留于 `.runtime/trainer-deployment/native-04bdd777/` 的 `tests.*`/`result.json` 与 `rerun-tests.*`/`rerun-result.json`；首次冷启动超时原因仍待完整生命周期验收定位。测试工具使用 uv.lock 中固定版本和 wheel 哈希安装，仅影响 quantx-train。SSH agent 再次通信失败后，显式本地 id_rsa 并设置 IdentityAgent=none/IdentitiesOnly=yes 验证连接和传输成功。
- 开发控制面：本机 `quantx_dev` 已通过 Alembic 从 0073 升至 0074，验证 source_bundle 为可空 JSON；创建 `quantx_trainer_dev` 并以真实登录通过现有数据库权限预检。创建时唯一既有登录角色为开发超级用户；已撤销 PUBLIC 对 quantx_dev 的 TEMPORARY、对 postgres/quantx_test 的 CONNECT，研究表权限严格按五表清单授予。私有连接信息仅写入忽略目录内的 0600 文件，不提交。开发 Prefect 已创建 Process Pool `quantx-train-pool`，UUID=`4e27aaf1-3f41-4858-8e05-95a66b08bae6`，未暂停、未启动 Worker。证据为 `.runtime/trainer-deployment/development-control-plane.json`、`development-role.json`、`development-pool.json`。此前开发容器只绑定回环；新增可选 `ops/config/compose.trainer-development.yaml` 并以实际 Compose 验证保留回环、只额外绑定显式开发 LAN IP 的两个端口、缺少 IP 拒绝解析。用户要求继续解决后，已重建这两个开发容器并从 Windows 验证 192.168.5.12:5432/4200 均可连接；实际镜像和命名数据卷前后一致，原 Pool UUID 和 Trainer 数据库权限复验通过。证据为 `.runtime/trainer-deployment/network-apply/`；Redis/InfluxDB 映射不变。
- Windows 运行身份与冻结：独立非管理员账户 `MYPC\quantx_trainer` 的代码/Conda 全树与祖先权限预检已实际通过。交互式凭据启动返回 0xC0000142；任务计划程序的安全日志明确显示批处理登录缺少权限（4625、0xc000015b），保留现有授权并仅补充该账户的 SeBatchLogonRight 后，Limited/Password 任务实际运行通过。凭据仍仅保存在部署身份的受限 DPAPI 文件中。远端完整 preflight 随后通过，覆盖运行身份、开发数据库、专用 Pool 与 SFTP；证据已回收至 `.runtime/trainer-deployment/windows-runtime/`。旧 ACL 会话已结束，不能继续把账户方案或重建授权列为阻塞。
- 开发存储：固定摘要的 `ops/config/compose.trainer-store.yaml` 已在 macOS 独立运行，192.168.5.12:2222、强制密钥认证及 chroot SFTP，仅挂载主机密钥、公钥、datasets/artifacts。重建后写入/改名/回读/清理及越界、转发、TTY 拒绝复验通过，见 `store-verification.json`。Mac Worker→Windows 低权限身份→Mac 完成 64 字节双向传输和哈希验证，见 `cross-transfer-verified.json` 及回收的运行端证据；这只是传输验收，不替代训练 bundle/行情闭环。Mac 当前开发 Worker 已加载私有 transfer 配置，准备与结果导入部署 READY，见 `dev-deployments-observed.json`。
- 主机策略实际部署：Windows ProgramData 与 macOS 固定系统路径策略已建立，运行身份不能修改策略；Windows 全树及父目录有效写权限检查通过，macOS root 所有及 sticky 目录保护已核验。两端实际 high_resource_guard 均拒绝工作日 09:15–16:30，返回 TRADING_OR_POST_CLOSE_CRITICAL_WINDOW；锁和运行证据保留。Windows 预算 CPU 2 线程、RSS 4096 MiB、最小空闲内存 8192 MiB、显存比例 0.5；Mac RSS 2048 MiB、最小空闲内存 4096 MiB。证据为 `windows-runtime/host-policy-probe.json` 与 `macos-host-policy-verified.json`。保护时段内不进行训练或 GPU 资格探测。
