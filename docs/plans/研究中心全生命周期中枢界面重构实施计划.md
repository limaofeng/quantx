...
| G01 | 跨研究类型统一运行连接                 | 实验运行               | 全局排序、过滤、分页和类型特定摘要               | 已实现：`researchLifecycleRuns` 唯一权威连接 |
...
- G01 已实现。唯一权威 GraphQL 连接字段为 `researchLifecycleRuns`，已完成
  schema、resolver、前端查询、生成类型、文档和测试的原子切换；不再以多个分页
  列表拼接模拟全局运行索引。
- 页面计划 P01–P07 均已实现，包含固定二级导航、训练准备度门禁、训练生命周期
  详情、统一实验运行索引、模型库和既有研究证据详情返回路径。
- 工程验证已完成：通过 Caddy `http://127.0.0.1:8080/graphql` 执行 codegen，
  `docs:contracts`、Web `check`、lint（0 error，仅任务外 TTrade 既有 warning）、
  Web 全量测试（156 files / 858 tests）、build 与 bundle budget、Ruff 和
  `git diff --check`。后端 G01 相关测试宽集合为 125 passed / 2 skipped；主代理
  最终四个明确受影响测试文件复验为 123 passed / 2 skipped。
- 根 `tests/` 结果为 4765 passed / 20 skipped / 32 failed。失败集中于任务外的
  Agent、Engine、交易流水线、QMT 当前工作区改动及集成状态，不涉及研究中心。
- 浏览器验收在 `profile=full`、`agentMode=live` 下完成：QMT 为 ready，
  `liveTrading` 为 enabled；G01 实际返回 14 条记录，搜索后收敛为 1 条。五个
  研究中心路由、standard/compact 密度和 1024px 准备度前置布局均已检查，控制台
  无新增 console warning/error；验收过程中未触发训练或交易。
- 与批准预览的必要偏差：当前 Web 没有真实指标研究创建 API，因此按钮保持
  disabled 并解释原因；本机当前没有 CERTIFIED 数据集、训练运行或模型，能力快照
  也不新鲜，所以运行态展示真实阻断和空状态，而不是预览中的示例成功数据。
  实现没有新增兼容层，也没有虚构 API。

## 17. 后续运行准备度任务

当前页面暴露的“没有认证数据集”“训练能力快照缺失或已过期”和“GPU 尚未通过
资格验证”属于运行准备度问题，不属于本界面重构的视觉缺陷。问题边界、依赖、
排查入口和完成条件见
[研究中心模型训练准备度三项问题任务说明](./研究中心模型训练准备度三项问题任务说明.md)。
