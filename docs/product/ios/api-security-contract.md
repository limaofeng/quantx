...
原生会话只按设备保存最小权限；唯一账户始终从用户账户授权关系解析。
`POST /auth/session` 请求为：
...
3. 服务端要求用户恰好授权一个资金账户；没有账户或账户数量大于一时登录失败，
   每次认证和敏感写入都重新校验该唯一关系，不在设备会话中重复保存账户。
4. Refresh Token 轮换保持同一设备会话和权限上限，不得借刷新扩权。
5. Access Token 至少绑定 `sub`、`sid`、权限、签发和过期时间；账户不是 Token
   claim，账户授权以数据库当前唯一关系为准。
