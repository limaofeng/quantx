...
| `IOS-TTR-002` | 做 T 待确认信号以 `TTradeSession.signalSnapshot` 为唯一确认真源，绑定 candidate fingerprint 与 signal/candidate/config/policy 版本后再走服务端预览、短时挑战和逐次生物确认；旧快照、未知协议和上下文错配均 fail-closed。尚未执行受控实盘，普通回测/PAPER 观察不作为控制授权证据。 | TTrade approval APIs；`strategy:read`,`trade:approve` | `T-TTRADE,T-SECURITY` / `E-AUTO`；缺 `E-LIVE` | `IMPLEMENTED_AUTO` |
