...
普通策略、做 T、买入计划和卖出计划共享同一个 LIVE 账户容量。运行内的额度和
`OrderSizer` 只是前置约束，不能作为最终入队凭证。创建命令时，
`AccountCapacityService` 在账户控制行锁及标的持仓行锁内读取同一份已处理、哈希和
分区完整性合法的协议 1.1 完整快照，扣除快照尚未覆盖的本地命令与保护义务，再与
pending/outbox 在同一事务内完成核验和写入。过期、缺失或不完整的快照不得放行。
保护量调整和批量清仓也先锁账户控制行、再锁持仓及计划，在该锁范围内核验库存与
其他保护，不能与命令入队并发争用同一容量。

券商快照中的可用资金/可卖量已经扣除了它所覆盖的活动订单；本地尚未被该快照
观察到的订单仍需额外占用。刷新旧快照不得释放这些预留，晚于快照的部分成交或终态
也不能反过来扩大该快照下的容量。精确重试返回原命令，不重复占用；改变关键委托字段
必须拒绝。LIVE BUY 必须提供可证明资金上限的 `FIX_PRICE` 限价。

正向做 T 的 ENTRY BUY 还必须受未被其他订单、T 批次或保护计划占用的老仓可卖量
约束；策略目标金额、人工确认或缺失可卖量都不能绕过这一限制。未退出的 T 成交继续
保留退出所需库存，领先于 TRADE 的委托 `FILLED` 不能提前释放。该约束在定量、审批后
执行和最终命令入队三个边界执行。

PAPER 策略运行不得自动读取 LIVE 持仓作为模拟初始仓位；其现金、订单、保护计划和
库存归属当前模拟运行。无运行的 PAPER 退出/清仓计划仅在创建时冻结持仓样本，之后
使用自身模拟库存，不能重新采样补仓或取代其他计划。PAPER 数据不参与 LIVE 容量、
授权预览、确认或执行判断；运行/计划的执行环境不可原地切换。

### 1.8 意图受理与回报归属必须明确

所有策略使用同一个严格的 `record_trade_intents` 受理边界：整批意图先持久化，之后才
能暴露给人工审批或继续路由。写入失败使运行失败，不允许以日志、内存缓存或缺省状态
替代受理。状态推进只能更新已受理意图，不能临时补建缺失记录。

托管买入回报同时匹配 `owner_type/owner_id`、`strategy_run_id`、计划、BUY 方向、
`intent_id` 和入场阶段。退出 SELL 不拥有入场状态机；同计划旧 BUY 的迟到成交仍应
计入累计执行事实，但不能清空当前阶段的 pending。恢复时确认“崩溃前从未形成订单”的
零成交事件也必须携带完整归属，不能为恢复路径放宽匹配规则。

### 1.9 完整 K 线与回放因果顺序

`StrategyInput.timestamp` 是行情可用/决策时间，`event.time` 保留原始存储标签，
`bar_period` 明确指标窗口所属周期。日期标记在零点的 `1d` 只在该交易日 15:00 后可用；
分钟线根据明确的起点/终点标签计算完成时间。未知周期或对齐规则直接拒绝。
实时形成中的 K 线不能作为完整 BAR 输入，预热也必须排除决策时点尚不可见的完整日线。

历史行情由统一排序的时间线推进，覆盖单/多标的及 Tick/BAR 组合；相同完成时刻的 Tick
先于 BAR，禁止由包含该 Tick 的完整 BAR 产生意图后再在该 Tick 上成交。Broker 回报由
独立串行消费者处理，每次撮合/退出评估/策略决策之后等待回报收敛，才处理下一行情。
历史行情不能排入该回报消费者，避免自等待和先推进价格后处理成交。所有 BACKTEST
必需回报、策略回调或检查点失败均进入失败路径，不能交付成功结果。

日线窗口只接受 `1d`；分钟输入可以使用最近完整日线指标，但不能推进 EMA、ATR、
持有天数或日线确认。超市策略仅接受所声明的日线。预热与正式处理按已保存的行情时间
防止重复累计；多标的预热同样按可用时间全局排序。

### 1.10 决策记忆与计划运行绑定

会影响下一次决策的算法记忆属于策略 `state`，不能仅放在 Python 对象属性中。当前
`decision_memory` 保存超市的箱体窗口、分析持仓、待入场标的及亏损计数，动态天平的
有界日线窗口及日线确认，网格的 EMA 递推种子、ATR 前收盘/真实波幅窗口和网格来源。
网格簿还必须保存触网最低价、触网时刻、部分成交与 pending 量；空网格/空库存是有效
状态，不得用启动参数重新填充。分析持仓不替代 Broker 持仓或公共可卖量。

`restore_algorithm_state()` 从已经安装的 `state` 重建私有缓存，在参数初始化后、
启动快照加载后以及持久回报回滚后执行；不能修改状态、输出意图或访问 I/O。恢复的
指标周期、递推种子和时间水位必须一致。缺日线确认时动态天平禁止盘中交易；缺本策略
持仓历史时超市禁止新入场；缺失或损坏的活动网格/库存状态必须阻断恢复。
检查点经 JSON 持久化再恢复后，相同后续输入应产生等价交易决策。

托管计划修改先保留旧有效运行绑定，仅在新配置版本对应的运行就绪后原子切换；失败时
旧绑定仍可控制和恢复。停止检查同时检查持久化交易义务，不能因为 runtime 不在内存中
就绕过未结订单、预留或退出保护。仍有保护义务时继续原运行的退出管理。
...
- 完整快照缺少逐账户状态证明，账户状态不是稳定的 `OK(0)` / `CLOSED(6)`，或
  状态在分区采集期间发生变化；
