...
export function useChartIndicators(
  ...
  subSeriesMapRef: React.MutableRefObject<Map<string, ISeriesApi<any>>>,
  isReady: boolean,
  chartVersion: number
) {
  // Keep track of active series to remove them later
  const mainSeriesMap = useRef<Map<string, ISeriesApi<any>[]>>(new Map());

  // Track sub series to cleanup
  // Map<subIndicatorType, { chart: IChartApi, series: ISeriesApi<any>[] }>
  ...
  useEffect(() => {
    ...
    activeSubs.forEach(type => {
      ...
      if (type === 'VOL') {
        ...
        seriesList.push(sMacd, sDiff, sDea);
        primarySeries = sMacd;
      } else if (type === 'KDJ') {
        chart.applyOptions({
          rightPriceScale: { scaleMargins: { top: 0.1, bottom: 0.1 } },
  ...
  }, [activeSubs, charts.subs, isReady, chartVersion]);
