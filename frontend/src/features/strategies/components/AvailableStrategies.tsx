...
export default function AvailableStrategies() {
  ...
  const strategies: StrategyDefinition[] = useMemo(
    () =>
      (((data as any)?.strategyDefinitions || []) as unknown[]).map(
          ...
          >
            ...
            <div className="relative p-6">
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-white/5">
                ...
                {/* 风险等级 */}
                <div
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border ${getRiskLevelColor(definition.riskLevel as any)}`}
                >
                  <Shield size={12} />
                  <span className="text-[10px] font-black uppercase tracking-tight">
                    {getRiskLevelName(definition.riskLevel as any)}
                  </span>
                </div>
              </div>
