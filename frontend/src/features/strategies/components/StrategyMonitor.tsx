...
export default function StrategyMonitor({
  ...
  const stockCode = useMemo(() => {
    if (!activeRun?.instruments?.length) return undefined;
  ...
  const overlays = useMemo(() => {
    ...
    try {
      ...
      const basePrice = params.basePrice || params.setup?.basePrice;
      if (basePrice) {
        list.push({
          type: 'priceLine',
      ...
      const levels =
        params.levels || params.setup?.levels || params.grid_levels;
      ...
      console.error('Failed to parse strategy parameters for monitor:', e);
      return [];
    }
  }, [activeRun]);
