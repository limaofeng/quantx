...
const GridChart: React.FC<Props> = ({ result, stockCode }) => {
  ...
  const chartData = useMemo(() => {
    ...
    return rawKlines.map((k: any) => ({
      time: k.time.split('T')[0], // 'YYYY-MM-DD' format
      open: k.open,
      high: k.high,
      low: k.low,
      close: k.close,
  ...
  useEffect(() => {
    ...
    const dataPrices = chartData.map((d: any) => d.value);
    ...
    if (chartData.length > 0) {
      const lastClose = (chartData[chartData.length - 1] as any).close;
    ...
    const handleVisibleRangeChange = () => {
      ...
      if (needsUpdate) {
        timeScale.setVisibleLogicalRange({ from, to } as any);
      }
    };
