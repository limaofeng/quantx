...
const getTimeMs = (value: unknown): number | null => {
  const date = parseMarketDate(value as any);
  const time = date?.getTime() ?? NaN;
  return Number.isFinite(time) ? time : null;
};

const getTickTimeMs = (tick: any): number | null => {
...
const buildCallAuctionTickBars = (ticks: any[]): any[] => {
...
const buildTickMinuteBar = (baseBars: any[], latestTick: any): any | null => {
  ...
  return {
    ...existing,
    stockCode: latestTick?.stockCode || existing?.stockCode,
    period: '1m',
...
const isKLineBaseReadyForTick = (baseBars: any[], latestTick: any) => {
  const tickMinuteMs = getMinuteMs(latestTick?.time);
...
export function useIntradayKLines(stockCode: string, mode: '1d' | '5d' = '1d') {
  ...
  const [klineMap, setKlineMap] = useState<Map<number, any>>(new Map());
  const [auctionTickMap, setAuctionTickMap] = useState<Map<number, any>>(
    new Map()
  );
  const [latestTick, setLatestTick] = useState<any | null>(null);

  const [klineSubResult] = useSubscription({
    query: MarketKLineSubscription as any,
  ...
  const data = useMemo(() => {
    ...
    const withCurrentData = new Map<number, any>();
