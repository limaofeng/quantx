...
function Invoke-RestoreVerificationSchemaGate {
  ...
  switch -CaseSensitive ($relation) {
    "current" {
      Write-Host "Restored database schema is already at the workspace head."
      break
    }
    "behind" {
      Write-Host (
        "Restored database schema is behind the workspace; upgrading only " +
      ...
      if ($LASTEXITCODE -ne 0) {
        throw (
          ...
          "configured database was not modified."
        )
      }
      break
    }
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "Restored database does not match the workspace Alembic head."
  }
}
...
function Invoke-MigrateAtRoot {
  ...
  try {
    ...
    if ($status.revision_relation -eq "incompatible") {
      throw (
        "Database revision is unknown or ahead of this workspace: " +
