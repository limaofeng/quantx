"""Versioned, point-in-time daily factors shared by screening and research.
...
def _factor(
  ...
  **kwargs: object,
  ...
  return FactorDefinition(id, label, category, formula, unit, lookback, **kwargs)


FACTOR_DEFINITIONS = (
  _factor("current_price", "收盘价", "价格", "当日未复权收盘价", "元", 1),
  _factor(
  ...
  _factor("avg_volume_5", "前5日均量", "量额", "前5日成交量均值（不含当日）", "手", 6),
  _factor(
...
FACTOR_BY_ID = {factor.id: factor for factor in FACTOR_DEFINITIONS}
...
def normalize_conditions(conditions: Sequence[Mapping[str, object]]) -> list[dict]:
  ...
  if len(conditions) > 64:
    raise ValueError("最多支持64个因子条件")
  normalized: dict[str, dict] = {}
  for condition in conditions:
    if set(condition) - {"factor_id", "operator", "value", "value_to"}:
      raise ValueError("因子条件包含未知字段")
    ...
    if operator not in factor.operators:
      raise ValueError(f"{factor_id} 不支持操作符 {operator}")
    value = _condition_number(condition.get("value"))
    item = {"factor_id": factor_id, "operator": operator, "value": value}
    if operator == "between":
    ...
    if factor.kind == "binary" and value not in (0.0, 1.0):
      raise ValueError("二值因子仅接受0或1")
...
def valid_factor_observations(bars: pd.DataFrame) -> pd.Series:
...
def calculate_factor_frame(
  ...
  *,
  ...
  so additional earlier history cannot change an already warm factor.
  ...
  if trading_dates is not None:
    ...
    if (
      ...
      raise ValueError("因子日线必须按唯一有效交易日升序排列")
    market_days = (
    ...
    if len(aligned_days) != len(source_days):
      ...
      calculated = calculate_factor_frame(aligned.reindex(aligned_days))
      return calculated.reindex(source_days).set_axis(bars.index)
  valid = valid_factor_observations(bars)
