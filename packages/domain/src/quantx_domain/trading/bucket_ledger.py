...
class BucketLedger:
  ...
  def validate_invariants(
    ...
    for code in sorted(instrument_codes):
      ...
      for field_name, actual in optional_fields.items():
        if field_name in position:
          checks[field_name] = (actual, int(position.get(field_name, 0) or 0))
      for field_name, (actual, expected) in checks.items():
        if actual != expected:
          issues.append(
            f"{code}.{field_name}: bucket={actual}, position={expected}"
          )
    return issues
