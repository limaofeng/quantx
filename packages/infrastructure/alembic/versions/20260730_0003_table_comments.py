...
REQUIRED_TABLE_COMMENTS: dict[str, str] = {
...
OPTIONAL_TABLE_COMMENTS: dict[str, str] = {
  # 该遗留表由复权因子仓储按需加载，不在生产基线元数据中。
  "divid_factors": "证券除权除息与复权因子",
}

TABLE_COMMENTS = REQUIRED_TABLE_COMMENTS | OPTIONAL_TABLE_COMMENTS


def upgrade() -> None:
  for table_name, comment in REQUIRED_TABLE_COMMENTS.items():
    op.create_table_comment(table_name, comment)
  for table_name, comment in OPTIONAL_TABLE_COMMENTS.items():
    op.execute(
      f"""
      DO $quantx_table_comment$
      BEGIN
