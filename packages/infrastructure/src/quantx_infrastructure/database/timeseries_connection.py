...
class NonRetryableWriteError(WriteError):
  """InfluxDB 服务端状态已损坏，继续写入只会放大故障。"""
...
_FATAL_WAL_ERROR_MARKERS = (
  "another process has written to the wal ahead of this one",
  "wal is shutdown",
)


def is_fatal_wal_error(error: object) -> bool:
  """Return whether an exception chain reports an unrecoverable Influx WAL state."""
  pending = [error]
  seen: set[int] = set()
  while pending:
    current = pending.pop()
    ...
    if isinstance(current, NonRetryableWriteError):
      return True
    message = str(current).casefold()
    if any(marker in message for marker in _FATAL_WAL_ERROR_MARKERS):
      return True
  ...
  def _create_client(self):
    ...
    try:
      client = _InfluxDBClient3(
        ...
        query_timeout=self.timeout * 1000,
