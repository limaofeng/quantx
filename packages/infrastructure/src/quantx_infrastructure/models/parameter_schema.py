...
def validate_strategy_configuration(
  ...
  """Merge strategy defaults and validate only strategy-owned parameters.

  Runtime parameters such as ``instrument_code`` and internal version markers
  are deliberately preserved outside the strategy schema. This keeps
  ``additionalProperties=False`` meaningful for the strategy-owned surface
  without rejecting execution-layer configuration.
  """

  schema_object = strategy_class.get_parameter_schema()
  if hasattr(schema_object, "model_dump"):
    schema = schema_object.model_dump(exclude_none=True)
  elif isinstance(schema_object, dict):
    schema = dict(schema_object)
  else:
    raise TypeError("strategy parameter schema must be a mapping or Pydantic model")

  properties = dict(schema.get("properties") or {})
  defaults = extract_default_parameters(schema)
  raw = dict(parameters or {})
  owned = {
    **defaults,
    **{key: raw[key] for key in properties if key in raw},
  }
  valid, error = validate_parameters(owned, schema)
  if not valid:
    raise ValueError(error or "策略参数校验失败")

  validator = getattr(strategy_class, "validate_configuration", None)
