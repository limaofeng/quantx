...
from quantx_infrastructure.database.timeseries_base import (
  BaseModel,
...
class Tick(BaseModel):
  ...
  def from_xtquant(stock_code: str, tick: Dict[str, Any]) -> "Tick":
    ...
    return Tick(
      ...
      price_tick=(
        tick.get("priceTick") or tick.get("PriceTick") or tick.get("price_tick") or 0.01
      ),
      up_stop_price=(
        tick.get("upperLimit")
        or tick.get("upStopPrice")
        or tick.get("UpStopPrice")
        or tick.get("up_stop_price")
        or 0.0
      ),
      down_stop_price=(
        tick.get("lowerLimit")
