...
class DailyAssetSnapshotRepository(BaseRepository[DailyAssetSnapshot]):
  ...
  async def find_page(
    ...
    *,
    ...
    first: int = 60,
  ) -> tuple[List[DailyAssetSnapshot], bool]:
    """Return an exact descending page using (trade_date, id) as cursor."""
    safe_first = max(1, min(int(first or 60), 100))
    stmt = select(DailyAssetSnapshot)
    if account_id:
      stmt = stmt.where(DailyAssetSnapshot.account_id == account_id)
    if strategy_run_id:
    ...
    if end_date:
      stmt = stmt.where(DailyAssetSnapshot.trade_date <= end_date)
    if cursor_date is not None and cursor_id:
      stmt = stmt.where(
        or_(
          DailyAssetSnapshot.trade_date < cursor_date,
          and_(
            DailyAssetSnapshot.trade_date == cursor_date,
            DailyAssetSnapshot.id < cursor_id,
          ),
        )
      )
    stmt = stmt.order_by(
      DailyAssetSnapshot.trade_date.desc(),
      DailyAssetSnapshot.id.desc(),
    ).limit(safe_first + 1)
