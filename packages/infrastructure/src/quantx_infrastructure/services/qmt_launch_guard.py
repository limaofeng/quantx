"""Process-local authority for the managed Windows QMT Agent launch."""
...
def qmt_agent_launch_block_reason() -> str | None:
  """Return the public reason when the Windows launcher skipped QMT startup."""
...
def qmt_heartbeat_matches_current_launch(updated_at: datetime | None) -> bool:
  """Return whether a heartbeat belongs to this managed Windows launch.

  Explicit launcher states fail closed. Unmanaged test and maintenance
  processes keep the established heartbeat behavior when no launch state is
  published.
  """
