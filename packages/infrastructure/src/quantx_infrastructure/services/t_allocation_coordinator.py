...
attempt, including recovery, is rebuilt from the current authoritative account cut.
"""
...
class TAllocationCoordinator:
  ...
  async def allocate_cycle(
    ...
    *,
    ...
    if snapshot.environment.value != self.environment:
      raise TAllocationConflict("T_ALLOCATION_SNAPSHOT_ENVIRONMENT_CONFLICT")
