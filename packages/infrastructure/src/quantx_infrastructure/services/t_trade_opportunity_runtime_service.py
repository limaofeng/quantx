...
class TTradeOpportunityRuntimeService:
    ...
    for raw_event in events:
      ...
      if record_kind == T_TRADE_EVALUATION_KIND_DIAGNOSTIC:
        ...
        preview = {
          **event,
          "record_kind": T_TRADE_EVALUATION_KIND_DIAGNOSTIC,
          "window_started_at_ms": (evaluated_at_ms // T_TRADE_DIAGNOSTIC_WINDOW_MS)
          * T_TRADE_DIAGNOSTIC_WINDOW_MS,
  ...
  def _validate_checkpoint_record_keys(records: Sequence[Mapping[str, Any]]) -> None:
    durable_keys = [
      _required_text(record.get("event_key"), "评估事件键") for record in records
    ]
  ...
  def _stage_diagnostic_event(
    ...
    *,
    ...
    inactive_keys = {
      ...
      if key != window_key
      and source_time_ms - existing.latest_source_time_ms >= self._diagnostic_idle_ms
    }
    keys_to_persist = closed_keys | inactive_keys
    if window_key not in windows:
      required = (
        len(windows) + 1 - (len(keys_to_persist) + self._max_diagnostic_windows)
      )
      if required > 0:
        available = [
          key for key in windows if key != window_key and key not in keys_to_persist
        ]
        available.sort(key=lambda key: self._diagnostic_window_sort_key(windows, key))
  ...
  async def load_reference_profile(
    ...
    *,
    ...
    opportunity replay on the same D-1 information set. A replay may also pin
    one immutable fingerprint prepared before the first Tick is consumed.
    """

    local_time = time_utils.to_shanghai(evaluated_at)
    cutoff = datetime.combine(local_time.date(), time.min) - timedelta(microseconds=1)
    if repository is not None:
      row = (
        await repository.get_by_fingerprint(
          instrument_code=instrument_code,
          fingerprint=required_fingerprint,
        )
        if required_fingerprint
        else await repository.latest_at_or_before(
          instrument_code=instrument_code,
          as_of=cutoff,
      ...
      async for db in get_async_db():
        ...
        row = (
          await profile_repository.get_by_fingerprint(
            instrument_code=instrument_code,
            fingerprint=required_fingerprint,
          )
          if required_fingerprint
          else await profile_repository.latest_at_or_before(
            instrument_code=instrument_code,
            as_of=cutoff,
            schema_version=T_TRADE_PROFILE_SCHEMA_VERSION,
            version=required_version,
    ...
    if row is None:
      return None
    if (
      time_utils.to_shanghai(row.as_of) > time_utils.to_shanghai(cutoff)
      or str(row.schema_version) != T_TRADE_PROFILE_SCHEMA_VERSION
      or (required_version is not None and str(row.version) != required_version)
    ):
      return None
    ...
    profile.update(
      {
        ...
        "as_of_trade_date": (time_utils.to_shanghai(row.as_of).date().isoformat()),
    ...
    *,
    ...
    async with self._diagnostic_lock:
      ...
      inactive_keys = {
        ...
        if key != window_key
        and source_time_ms - pending.latest_source_time_ms >= self._diagnostic_idle_ms
      }
      keys_to_persist = closed_keys | inactive_keys
      if window_key not in self._diagnostic_windows:
        required = (
          len(self._diagnostic_windows)
          + 1
    ...
    *,
    ...
    async with self._diagnostic_lock:
      closed_keys = {
        ...
        and pending.window_started_at_ms + T_TRADE_DIAGNOSTIC_WINDOW_MS <= through_ms
      }
      inactive_keys = {
        key
        for key, pending in self._diagnostic_windows.items()
  ...
  def _pending_diagnostic_event_keys(
    ...
    *,
    ...
    ordered: list[str] = []
    for window in pending:
      if (account_id is not None and window.account_id != account_id) or (
        strategy_run_id is not None and window.strategy_run_id != strategy_run_id
      ):
        continue
  ...
  def _normalize_evaluation_event(
    ...
    *,
    ...
    normalized: dict[str, Any] = {
      ...
      "policy_version": _required_text(snapshot.get("policy_version"), "机会策略版本"),
      "schema_version": str(
      ...
      normalized.update(
        {
          "window_started_at": _from_epoch_ms(
            _positive_int(event.get("window_started_at_ms"), "诊断窗口开始时间")
          ),
          "window_ended_at": _from_epoch_ms(
            _positive_int(event.get("window_ended_at_ms"), "诊断窗口结束时间")
          ),
