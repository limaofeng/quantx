...
def test_metrics_middleware_initialization():
    """测试指标中间件初始化"""
    from quantx_api.monitoring import MetricsMiddleware
    ...
    assert not middleware_disabled.enabled

def test_application_metrics():
    """测试应用指标管理器"""
    from quantx_api.monitoring import ApplicationMetrics
...
def test_system_metrics(mock_disk, mock_memory, mock_cpu):
    """测试系统指标收集"""
    from quantx_api.monitoring import SystemMetrics
