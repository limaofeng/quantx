...
@pytest.mark.asyncio
async def test_stock_disclosure_summary_returns_empty_shape(
...
@pytest.mark.asyncio
async def test_stock_disclosure_summary_returns_failed_sync_status(
  monkeypatch, authorized_graphql_context
):
  result = await _execute_summary(
    monkeypatch,
...
@pytest.mark.asyncio
async def test_stock_disclosure_summary_returns_announcement_only(
  monkeypatch, authorized_graphql_context
):
  result = await _execute_summary(
    monkeypatch,
...
@pytest.mark.asyncio
async def test_stock_disclosure_summary_returns_repurchase_only(
  monkeypatch, authorized_graphql_context
):
  result = await _execute_summary(
    monkeypatch,
...
async def _execute_summary(monkeypatch, context, summary_data):
  from quantx_api.gqlapi.resolvers.announcements import AnnouncementResolver
  from quantx_api.gqlapi.schema import schema
  from quantx_api.gqlapi.types.announcement_types import StockDisclosureSummary
