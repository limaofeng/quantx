"""The indicator formula contract is shared, finite-window and strictly causal."""
...
def test_prefix_and_finite_warmup_invariance():
  frame = bars()
  all_values = calculate_indicator_frame(frame)
  pd.testing.assert_frame_equal(
    all_values.iloc[:290], calculate_indicator_frame(frame.iloc[:290])
  )
  # Enough local history is identical to loading the whole listing history.
  pd.testing.assert_series_equal(
    all_values.iloc[-1], calculate_indicator_frame(frame.iloc[-260:]).iloc[-1]
  )
...
def test_physical_missing_session_matches_explicit_nan_without_future_rows():
  ...
  calculated = calculate_indicator_frame(sparse, trading_dates=frame.time)
  pd.testing.assert_frame_equal(
    calculated, calculate_indicator_frame(explicit).drop(index=missing_index)
  )
  ...
  pd.testing.assert_frame_equal(
    calculated.iloc[:-2],
    calculate_indicator_frame(prefix, trading_dates=frame.time),
  )
...
def test_nullable_quality_flags_conservatively_reject_missing_calendar_rows():
  ...
  calculated = calculate_indicator_frame(frame.drop(index=37), trading_dates=frame.time)
  assert pd.isna(calculated.iloc[-1].ma5)
  assert calculated.iloc[-1].current_price == pytest.approx(frame.close.iloc[-1])
  frame.loc[39, "suspend_flag"] = pd.NA
  assert pd.isna(calculate_indicator_frame(frame).iloc[-1].current_price)
...
def test_true_falling_streak_and_zero_rsi_are_preserved():
  ...
  falling = calculate_indicator_frame(frame).iloc[-1]
  ...
  rising = calculate_indicator_frame(frame).iloc[-1]
  ...
  assert calculate_indicator_frame(frame).iloc[-1].consecutive_down_days == 0


def test_missing_history_denominators_and_bad_values_do_not_fabricate_indicators():
  frame = bars(40)
  frame["volume"] = 0
  frame["amount"] = np.nan
  value = calculate_indicator_frame(frame).iloc[-1]
...
def test_condition_identity_zeros_boundaries_and_strict_errors():
  low = {"indicator_id": "rsi12", "operator": "lte", "value": 0}
  high = {
  ...
  for invalid in (
    {**low, "indicator_id": "unknown"},
    {**low, "value": np.inf},
    {**low, "operator": "oops"},
    {**high, "value_to": 0},
    {"indicator_id": "kdj_cross_up", "operator": "eq", "value": 2},
  ):
...
def test_price_units_and_snapshot_parity():
  ...
  calculated = calculate_indicator_frame(frame).iloc[-1]
  snapshot = build_snapshot_record(
    "000001.SZ", "stock", "示例", frame.time.iloc[-1].date(), frame
  )
  assert snapshot["calculation_version"] == INDICATOR_VERSION
  assert snapshot["valid_history_count"] == len(frame)
