...
def _readiness(
  *,
  ...
  checks = [
    ...
    ("ENGINE_READY", "PREPARATION"),
    ("LIVE_AGENT_READY", "PREPARATION"),
    ("AGENT_MODE_LIVE", "PREPARATION"),
    ("PROTOCOL_1_1", "PREPARATION"),
    ("ROLLOUT_CONFIGURED", "PREPARATION"),
    ("SNAPSHOT_RECONCILED", "PREPARATION"),
    ("SNAPSHOT_FRESH", "PREPARATION"),
    ("SNAPSHOT_ACTIVITY_CLASSIFIED", "PREPARATION"),
    ("RECENT_BACKUP", "AUTOMATION"),
    ("NO_CRITICAL_ALERTS", "PREPARATION"),
    ("NO_DEAD_LETTERS", "PREPARATION"),
    ("CONTROLLED_WINDOW_ACTIVE", "AUTOMATION"),
    ("NO_EXTERNAL_BROKER_ACTIVITY", "AUTOMATION"),
    ("KILL_SWITCH_CLEAR", "PREPARATION"),
  ]
  return {
...
def test_t_trade_switch_does_not_change_account_execution_capability() -> None:
  ...
  assert all(
    item["code"] != "T_TRADE_LIVE_ENABLED" for item in status["checks"]
  )


def test_missing_account_window_is_a_healthy_reduce_only_state() -> None:
  status = project_account_execution_safety(
    _readiness(failed={"CONTROLLED_WINDOW_ACTIVE"}, stage="SHADOW")
  )
...
def test_kill_switch_is_explicit_while_preserving_reduction_capability() -> None:
  status = project_account_execution_safety(
    _readiness(failed={"KILL_SWITCH_CLEAR"}, kill_switch=True)
  )
