"""New indicator calculations never silently adopt old snapshots."""
...
def test_additive_indicator_version_migration_does_not_promote_legacy_rows(monkeypatch):
  path = (
    Path(__file__).resolve().parents[2]
    / "packages/infrastructure/alembic/versions/20260901_0042_daily_indicator_version.py"
  )
  spec = importlib.util.spec_from_file_location("daily_indicator_migration", path)
