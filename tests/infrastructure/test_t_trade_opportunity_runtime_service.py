...
async def test_materialize_diagnostic_coalesces_latest_event_in_source_time_window():
  ...
  assert kwargs["event_key"] == ("run-1:600000.SH:DIAGNOSTIC:1724300002000")
...
def _diagnostic_event(
  ...
  *,
  ...
  return {
    "type": "T_TRADE_OPPORTUNITY_EVALUATION",
    "event_key": (f"run-1:{instrument_code}:{evaluated_at_ms}:{ordinal}:DIAGNOSTIC"),
    "record_kind": "COALESCED_DIAGNOSTIC",
    "event_type": "HEARTBEAT",
...
async def test_diagnostic_windows_are_bounded_and_evict_sparse_streams_by_source_time():
  ...
  assert set(
    repository.append_coalesced_diagnostic.await_args_list[i].kwargs["instrument_code"]
    for i in range(3)
  ) == {
    "000001.SZ",
    "300001.SZ",
...
async def test_checkpoint_batch_failure_has_no_partial_commit_and_retries_preaggregated_count(
  ...
  monkeypatch.setattr(
    TTradeOpportunityEvaluationRepository, "append_many", fail_once_then_append
  )
  service = TTradeOpportunityRuntimeService()
  first = _diagnostic_event(
    "600000.SH",
...
@pytest.mark.asyncio
async def test_profile_loader_uses_frozen_fingerprint_instead_of_latest_profile():
  row = SimpleNamespace(
    as_of=datetime(2026, 8, 22, 15, 5),
    profile={
      "pullback_threshold_pct": 0.8,
  ...
  repository = SimpleNamespace(
    get_by_fingerprint=AsyncMock(return_value=row),
    latest_at_or_before=AsyncMock(),
  )

  result = await TTradeOpportunityRuntimeService().load_reference_profile(
    instrument_code="600000.SH",
    evaluated_at=datetime(2026, 8, 23, 10, 0),
    required_fingerprint="d" * 64,
    repository=repository,
  )

  repository.get_by_fingerprint.assert_awaited_once_with(
    instrument_code="600000.SH",
    fingerprint="d" * 64,
  )
  repository.latest_at_or_before.assert_not_awaited()
  assert result is not None
  assert result["profile_fingerprint"] == "d" * 64
