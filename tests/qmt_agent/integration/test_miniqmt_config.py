...
def _temp_config(tmp_path):
    ...
    from quantx_qmt_agent.miniqmt.config.config_manager import XTQuantConfig

    return XTQuantConfig(str(tmp_path / "quantx_infrastructure.config.json"))
...
def test_data_config(tmp_path):
    ...
    assert data_config['cache_enabled']
...
def test_xt_config_singleton():
    """测试配置单例"""
    from quantx_qmt_agent.miniqmt.config.config_manager import xt_config
...
class TestXTQuantConfigIntegration:
    ...
    def test_config_update_and_persist(self):
        ...
        try:
            # 创建并修改配置
            config = XTQuantConfig(temp_file)
            config.get('xtquant.data_server.host', 'localhost')
