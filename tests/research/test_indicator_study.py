...
import pandas as pd
import pytest
import quantx_research.indicator_runner as indicator_runner_module
import quantx_research.indicator_study as indicator_study_module
import quantx_research.runner as runner_module
import yaml
from quantx_domain.indicators import INDICATOR_DEFINITIONS, calculate_indicator_frame
...
from quantx_research.indicator_runner import (
  build_indicator_partitions,
  indicator_data_fingerprint,
)
...
def test_indicator_config_normalizes_identity_and_rejects_unsupported_filters() -> None:
  conditions = [
    {"indicator_id": "volume_ratio", "operator": "gte", "value": 0},
    {"indicator_id": "rsi6", "operator": "between", "value": 0, "value_to": 40},
  ]
  first = IndicatorStudyConfig(conditions=conditions)
  second = IndicatorStudyConfig(conditions=conditions[::-1] + conditions)
  ...
  assert report_match_key(
    "joint",
    list(first.required_indicator_ids),
    list(first.conditions),
    first.universe.identity(),
  ) == report_match_key(
    "joint",
    list(second.required_indicator_ids),
    list(second.conditions),
    second.universe.identity(),
  )
  first_universe = IndicatorStudyConfig(
  ...
  for payload in (
    {"indicator_ids": ["roe_ttm"]},
    {"indicator_ids": ["volume_ratio"], "outcomes": {"horizons": [61]}},
    {
      "indicator_ids": ["volume_ratio"],
      "statistics": {"bootstrap_samples": 20_001},
    },
    {"indicator_ids": ["volume_ratio"], "universe": {"exclude_st": True}},
    {"indicator_ids": ["volume_ratio"], "universe": {"include_industries": ["银行"]}},
    {
      "conditions": [
        {"indicator_id": "volume_ratio", "operator": "gte", "value": float("nan")}
      ]
    },
  ):
    with pytest.raises(ValueError):
      IndicatorStudyConfig.model_validate(payload)


def test_default_indicator_configs_cover_every_researchable_definition() -> None:
  root = Path(__file__).resolve().parents[2]
  wanted = {
    indicator.id for indicator in INDICATOR_DEFINITIONS if indicator.research_supported
  }
  for filename in ("indicator_study_v1.yaml", "indicator_study_v1_20260729.yaml"):
    payload = yaml.safe_load(
      (root / "apps/research/configs" / filename).read_text(encoding="utf-8")
    )
    config = IndicatorStudyConfig.model_validate(payload)
    assert set(config.indicator_ids) == wanted
    assert len(config.conditions) == 2


def test_data_fingerprint_includes_canonical_indicator_and_outcome_calendars() -> None:
  calendar = pd.bdate_range("2025-01-01", periods=5)
  original = indicator_data_fingerprint("a" * 64, calendar, calendar)
  assert original == indicator_data_fingerprint(
    "a" * 64, calendar[::-1].append(calendar), calendar
  )
  assert original != indicator_data_fingerprint("a" * 64, calendar.delete(2), calendar)
  assert original != indicator_data_fingerprint("a" * 64, calendar, calendar.delete(2))
  assert original != indicator_data_fingerprint("b" * 64, calendar, calendar)


def test_quantiles_never_split_ties_and_constant_values_do_not_make_five_groups() -> (
  None
):
  groups = indicator_groups(pd.Series([1, 1, 1, 2, 3, 4, 5]), binary=False)
  assert len(set(groups.iloc[:3])) == 1
  assert indicator_groups(pd.Series([7] * 10), binary=False).tolist() == ["Q3"] * 10
  assert indicator_groups(pd.Series([0, 1, np.nan]), binary=True).iloc[:2].tolist() == [
    "false",
    "true",
  ]
...
def test_outcomes_use_market_sessions_and_independent_horizon_completeness() -> None:
  ...
  result = add_indicator_outcomes(frame, dates, (1, 2, 4, 20))
...
def _config(path: Path, *, conditions: bool = True) -> Path:
  payload = {
    ...
    "conditions": [
      {"indicator_id": "volume_ratio", "operator": "gte", "value": 1},
      {"indicator_id": "change_pct", "operator": "gt", "value": 0},
    ]
    if conditions
    else [],
...
@pytest.mark.asyncio
async def test_indicator_runner_produces_reproducible_reports_and_keeps_short_outcomes(
  tmp_path: Path,
) -> None:
  config_path = _config(tmp_path / "indicator.yaml")
  ...
  assert metrics["indicator_version"] == "daily-indicator-v1"
  ...
  tampered = before.replace(
    b'"indicator_version": "daily-indicator-v1"',
    b'"indicator_version": "daily-indicator-v0"',
  )
  ...
  assert ".indicator-study-run-lease.json" not in {
  ...
  for artifact in rendered_manifest["artifacts"]:
    if artifact["path"] == "metrics.json":
      artifact.update(
        ...
        sha256=indicator_runner_module.file_sha256(metrics_path),
      )
  ...
  active_lease = indicator_runner_module._acquire_indicator_run_lease(
    run_dir,
...
def test_statistics_engine_identity_binds_source_and_runtime_versions() -> None:
  engine = indicator_runner_module._indicator_statistics_engine_identity()
  ...
  for source in source_files.values():
    assert source["sha256"] == indicator_runner_module.file_sha256(
      runner_module.REPO_ROOT / source["path"]
    )
  assert engine["source_sha256"] == indicator_runner_module.fingerprint(
    engine["source_files"]
  )
  ...
  assert engine["engine_sha256"] == indicator_runner_module.fingerprint(payload)


def _checkpoint_report(indicator_id: str, p_value: float) -> dict[str, object]:
  return {
    "report_id": f"single-{indicator_id}",
    "kind": "single",
    "indicator_ids": [indicator_id],
    "conditions": [],
    "match_key": indicator_id,
    "coverage": {},
    "distribution": [],
    "rows": [
      {
...
def test_indicator_report_checkpoints_resume_before_one_global_fdr(
  tmp_path: Path, monkeypatch
) -> None:
  config = IndicatorStudyConfig(
    indicator_ids=["change_pct", "volume_ratio"],
    date_range=("2025-01-01", "2025-06-30"),
    outcomes={"horizons": [1]},
  ...
  identity = {
    ...
    "statistics_engine": indicator_runner_module._indicator_statistics_engine_identity(),
  }
  calls: list[str] = []

  def interrupted(*args, indicator_ids, **kwargs):
    calls.append(indicator_ids[0])
    if len(calls) == 2:
      raise RuntimeError("interrupted after one completed report")
    return _checkpoint_report(indicator_ids[0], 0.01)

  monkeypatch.setattr(indicator_study_module, "_analyze_report", interrupted)
  ...
  def resumed(*args, indicator_ids, **kwargs):
    resumed_calls.append(indicator_ids[0])
    return _checkpoint_report(indicator_ids[0], 0.02)

  progress: list[tuple[int, bool]] = []
  monkeypatch.setattr(indicator_study_module, "_analyze_report", resumed)
  with RuntimeMemoryMonitor(reserve_gib=1) as monitor:
  ...
  source_changed["statistics_engine"]["source_sha256"] = (
    indicator_runner_module.fingerprint(
  ...
  for changed_identity in (source_changed, dependency_changed):
    ...
    engine["engine_sha256"] = indicator_runner_module.fingerprint(
...
async def test_legacy_statistics_identity_upgrades_only_without_checkpoints(
  ...
  config_path = _config(tmp_path / "legacy-indicator.yaml")
  run_dir = await run_study(
    config_path,
    source=FakeResearchSource(_market_bars()),
    output_root=tmp_path / "runs",
...
async def test_resume_rejects_changed_current_engine_with_or_without_checkpoints(
  ...
  config_path = _config(tmp_path / "changed-engine-indicator.yaml")
  run_dir = await run_study(
    config_path,
    source=FakeResearchSource(_market_bars()),
    output_root=tmp_path / "runs",
  ...
  changed_engine["engine_sha256"] = indicator_runner_module.fingerprint(
    {key: value for key, value in changed_engine.items() if key != "engine_sha256"}
  )
  monkeypatch.setattr(
    indicator_runner_module,
    "_indicator_statistics_engine_identity",
...
def test_indicator_resume_requires_consistent_schema_v2_coverage_identity() -> None:
  valid = {
    "is_complete": True,
  ...
  indicator_runner_module._validate_resume_dividend_factor_coverage(
  ...
  for quality in malformed:
    with pytest.raises(ValueError, match="schema-v2"):
      indicator_runner_module._validate_resume_dividend_factor_coverage(quality)
...
async def test_resume_rejects_indexed_legacy_dividend_coverage_before_statistics(
  ...
  config_path = _config(tmp_path / "legacy-coverage-indicator.yaml")
  run_dir = await run_study(
    config_path,
    source=FakeResearchSource(_market_bars()),
    output_root=tmp_path / "runs",
  ...
  for artifact in manifest["artifacts"]:
    if artifact["path"] == "data-quality.json":
      artifact.update(
        ...
        sha256=indicator_runner_module.file_sha256(quality_path),
      )
  manifest_path.write_text(
    json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8"
  )
  ...
  assert indexed["data-quality.json"]["sha256"] == (
    indicator_runner_module.file_sha256(quality_path)
  )
  assert indexed["analysis-sample.parquet"]["sha256"] == (
    indicator_runner_module.file_sha256(sample_path)
  )
  ...
  monkeypatch.setattr(
    indicator_runner_module, "analyze_indicator_partitions", must_not_analyze
  )
  with pytest.raises(ValueError, match="schema-v2"):
    await run_study(config_path, resume_run_dir=run_dir)
...
@pytest.mark.asyncio
async def test_failed_indicator_run_resumes_only_from_bound_frozen_checkpoints(
  tmp_path: Path, monkeypatch
) -> None:
  config_path = _config(tmp_path / "resume-indicator.yaml")
  run_dir = await run_study(
    config_path,
    source=FakeResearchSource(_market_bars()),
    output_root=tmp_path / "runs",
  ...
  repartition_sample = indicator_runner_module._partition_existing_indicator_sample
  ...
  monkeypatch.setattr(
    indicator_runner_module,
    "_partition_existing_indicator_sample",
    must_not_repartition,
  )
  resumed = await run_study(config_path, resume_run_dir=run_dir)
  assert resumed == run_dir
  ...
  # report.  Market data and indicator/outcome construction stay out of recovery.
  ...
  analyzed: list[str] = []
  analyze_report = indicator_study_module._analyze_report

  def record_analysis(*args, indicator_ids, **kwargs):
    analyzed.append(indicator_ids[0])
    return analyze_report(*args, indicator_ids=indicator_ids, **kwargs)

  monkeypatch.setattr(
    indicator_runner_module, "_partition_existing_indicator_sample", repartition_sample
  )
  monkeypatch.setattr(indicator_study_module, "_analyze_report", record_analysis)
  await run_study(config_path, resume_run_dir=run_dir)
  ...
  stale_attempt = indicator_runner_module._current_attempt_identity(
  ...
  lease_path = run_dir / ".indicator-study-run-lease.json"
  ...
  live_marker = indicator_runner_module._current_attempt_identity(
    run_dir, "live-marker"
  )
  ...
  changed_path = tmp_path / "changed-indicator.yaml"
...
@pytest.mark.asyncio
async def test_keyboard_interrupt_marks_indicator_resume_failed_and_releases_lease(
  tmp_path: Path, monkeypatch
) -> None:
  config_path = _config(tmp_path / "interrupted-indicator.yaml")
  run_dir = await run_study(
    config_path,
    source=FakeResearchSource(_market_bars()),
    output_root=tmp_path / "runs",
  ...
  monkeypatch.setattr(
    indicator_runner_module, "analyze_indicator_partitions", interrupt
  )
  ...
  probe = indicator_runner_module._acquire_indicator_run_lease(
    run_dir,
...
async def test_real_task_cancellation_is_terminal_resumable_and_keeps_quality_frozen(
  ...
  config_path = _config(tmp_path / "cancelled-indicator.yaml")
  run_dir = await run_study(
    config_path,
    source=FakeResearchSource(_market_bars()),
    output_root=tmp_path / "runs",
  ...
  frozen_quality_sha256 = indicator_runner_module.file_sha256(quality_path)

  analyze_partitions = indicator_runner_module.analyze_indicator_partitions
  ...
  monkeypatch.setattr(
    indicator_runner_module, "analyze_indicator_partitions", analyze_partitions
  )
...
@pytest.mark.asyncio
async def test_shared_indicator_values_and_quality_gate(tmp_path: Path) -> None:
  source = FakeResearchSource(_market_bars())
  config = IndicatorStudyConfig(
    indicator_ids=["volume_ratio", "change_pct"],
    date_range=("2025-01-01", "2025-06-30"),
  ...
  with RuntimeMemoryMonitor(reserve_gib=1) as monitor:
    partitions, quality = await build_indicator_partitions(
  ...
  expected = calculate_indicator_frame(raw).set_index(raw.time)
...
@pytest.mark.asyncio
async def test_bad_bar_invalidates_indicator_window_instead_of_bridging_gap(
  tmp_path: Path,
) -> None:
  bars = _market_bars()
  ...
  with RuntimeMemoryMonitor(reserve_gib=1) as monitor:
    partitions, _ = await build_indicator_partitions(
...
@pytest.mark.asyncio
async def test_physically_missing_bar_has_same_indicator_results_as_explicit_invalid_bar(
  tmp_path: Path,
) -> None:
  bars = _market_bars()
  ...
  config = IndicatorStudyConfig(
    indicator_ids=["ma5", "rsi6", "volume_ratio"],
    date_range=("2025-01-01", "2025-01-31"),
...
async def test_runner_releases_source_before_cpu_outcomes(
  ...
  complete = indicator_runner_module.finish_indicator_partitions
  ...
  monkeypatch.setattr(
    indicator_runner_module, "finish_indicator_partitions", checked_complete
  )
  await run_study(
...
def test_latest_year_is_trailing_twelve_months_with_exact_median_and_stock_count(
  ...
  cell.flush()
  config = IndicatorStudyConfig(indicator_ids=["volume_ratio"])
  row = _cell_result(
    cell,
    cell,
